...
#
#  What this bridge exposes to the AI chat provider (aggregated into one tools/list):
...
HOST = "127.0.0.1"
# Keep in sync with corelua-extension/manifest.json "version" - printed at
# startup so a user's terminal output alone tells us which build they're on.
BRIDGE_VERSION = "1.0"
PORT = int(os.environ.get("CL_BRIDGE_PORT", "47170"))
HERE = os.path.dirname(os.path.abspath(__file__))
CONFIG_PATH = os.path.join(HERE, "config.json")

# The primary server. It is always present, added by the installer, and can
# never be edited/removed through the extension (it is what CoreLua is FOR).
...
def log(msg, color="dim", terminal=True, level="INFO"):
    ...
    level: a short severity tag (INFO/WARN/ERROR/DEBUG) prepended to the file
    log line so the log is greppable by severity and easy to triage in a
    bug report. The terminal keeps its colour-driven readability and does
    not print the tag (the colour IS the severity signal there)."""
    if terminal:
        _clear_spinner_line()
    ...
    if _log_file:
        try:
            _log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} [{level:5}] {msg}\n")
            _log_file.flush()
        except Exception:
            pass


def log_warn(msg, terminal=True):
    """Convenience wrapper for warnings (yellow)."""
    log(msg, color="yl", terminal=terminal, level="WARN")


def log_err(msg, terminal=True):
    """Convenience wrapper for errors (red). Always goes to both terminal and
    file — an error that only lands in the file is invisible to a user who
    just screenshots the console."""
    log(msg, color="rd", terminal=terminal, level="ERROR")


def log_dbg(msg):
    """Debug/trace detail — file-only by default so the console stays clean."""
    log(msg, color="dim", terminal=False, level="DEBUG")
...
def _reclaim_studio_port(client):
    ...
    a zombie StudioMCP.exe from a crashed session keeps LISTENING on 47171.
...
def _reclaim_bridge_port():
    """Free OUR OWN listen port (47170) from a leftover bridge before we bind.
    ...
    (some unrelated python listening on 47170): the guard is the command line
    ...
    # the port alone would murder whatever legitimately owns 47170.
    ...
    if "bridge.py" not in cmdline.lower():
        log(f"port {PORT} is held by pid {pid_i} ('{name}') but it does not look "
            f"like a CoreLua bridge - leaving it alone.", "yl")
        return False
    log(f"port {PORT} is held by a leftover CoreLua bridge (pid {pid_i}) from a "
...
def check_studio_port():
    ...
    A third-party tool (e.g. "ropilot") that binds 47171 before Studio does
...
def _validate_config(cfg):
    """Validate the structure of a parsed config dict. Returns a list of
    human-readable warning strings (empty list = fully valid). Non-fatal: a
    malformed server entry is skipped at load time, not crashed on — but the
    warnings are logged so a mis-edited config.json is immediately visible."""
    warns = []
    if not isinstance(cfg, dict):
        return ["config.json root is not a JSON object"]
    servers = cfg.get("mcpServers", {})
    if not isinstance(servers, dict):
        warns.append("'mcpServers' is not a JSON object — no servers will load")
        return warns
    for sid, spec in servers.items():
        if not isinstance(sid, str) or not sid.strip():
            warns.append(f"server key '{sid}' is empty/invalid — skipped")
            continue
        if not isinstance(spec, dict):
            warns.append(f"server '{sid}' spec is not a JSON object — skipped")
            continue
        cmd = spec.get("command")
        if not isinstance(cmd, str) or not cmd.strip():
            warns.append(f"server '{sid}' has no 'command' string — skipped")
            continue
        args = spec.get("args", [])
        if not isinstance(args, list):
            warns.append(f"server '{sid}' 'args' is not a JSON array — treated as empty")
        env = spec.get("env")
        if env is not None and not isinstance(env, dict):
            warns.append(f"server '{sid}' 'env' is not a JSON object — ignored")
    return warns
...
def _read_config():
    if os.path.exists(CONFIG_PATH):
        try:
            ...
            log_err(f"config.json unreadable ({e}) - starting from a fresh one")
...
class MCPClient:
        ...
        with self.start_lock:
            ...
            with _Spinner(f"    [{self.id}] starting..."):
                ...
                self._request("initialize", {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "corelua-bridge", "version": "1.0"},
                }, timeout=30)
                ...
                #
                # Same class of bug, one level up: StudioMCP's catalogue can also
                # GROW after the first non-empty answer (reported live 2026-08-10:
                # boot banner said "ready 27 tools available - Roblox Studio
                # connected" while Studio eventually advertises 41). The old loop
                # broke on the FIRST non-empty list, so the cache was pinned at the
                # partial count (27) for the whole session - the terminal, the
                # popup and list_commands all under-reported until a manual
                # refresh. Keep polling until the count is STABLE (two consecutive
                # identical non-empty responses), not merely non-empty. A list that
                # is still growing (27 -> 41) keeps the loop running; a settled
                # catalogue breaks out after one confirmation poll.
                last_count = 0
                settle = 0
                for _ in range(12):
                    n = len(self.refresh_tools(timeout=3))
                    if n > 0:
                        if n == last_count:
                            settle += 1
                            if settle >= 2:
                                break  # same count twice in a row -> catalogue settled
                        else:
                            settle = 0
                        last_count = n
    ...
    def _write_json(self, payload):
        """Write one JSON-RPC frame and turn a disappearing child into a
        useful bridge error.

        A StudioMCP process can exit between ``is_alive()`` and the actual
        write.  The old code let BrokenPipeError escape from this race, which
        made a normal "Studio is not installed/open yet" state look like an
        unexplained bridge crash.
        """
        with self.write_lock:
            proc = self.proc
            if proc is None or proc.poll() is not None or proc.stdin is None:
                raise RuntimeError(f"server '{self.id}' closed before accepting the request")
            try:
                proc.stdin.write(json.dumps(payload) + "\n")
                proc.stdin.flush()
            except (BrokenPipeError, ConnectionResetError, OSError) as exc:
                raise RuntimeError(
                    f"server '{self.id}' closed its input pipe: {exc}"
                ) from exc
...
def safe_call(name, arguments, timeout):
    ...
    Error messages include the tool name, the kind of failure, and a hint where
    possible, so the AI (and the user reading the log) can self-correct instead
    of staring at a bare exception string."""
    try:
        result = mgr.call(name, arguments, timeout)
        return {"ok": True, "text": result["text"], "images": result["images"]}
    except TimeoutError as e:
        msg = str(e) if str(e) else f"tool '{name}' timed out after {timeout:.0f}s"
        log_err(f"tool '{name}' TIMEOUT after {timeout:.0f}s: {msg}")
        return {"ok": False, "error": f"Timeout: {msg}. The MCP server (or Roblox Studio) did not respond in time. If Studio is busy (e.g. running a long script), retry with a larger timeout.", "kind": "timeout"}
    except KeyError:
        log_err(f"tool '{name}' not found in any connected MCP server")
        return {"ok": False, "error": f"Tool '{name}' is not available. It may belong to an MCP server that is currently down — check the server list in the popup.", "kind": "tool_not_found"}
    except Exception as e:
        kind = type(e).__name__
        msg = str(e) if str(e) else kind
        log_err(f"tool '{name}' failed ({kind}): {msg}")
        return {"ok": False, "error": f"[{kind}] {msg}", "kind": kind}
    ...
    try:
        ...
        async for raw in ws:
            try:
                ...
                log_dbg(f"dropped non-JSON frame from extension: {raw[:120]!r}")
                continue
            mtype = msg.get("type")
            rid = msg.get("id")
        ...
        log_err(f"handler error ({type(e).__name__}): {e}")
    finally:
        clients.discard(ws)
    ...
    while True:
        ...
        if rc0 is not None and rc0.is_alive() and not rc0.tools_cache:
            ...
            if got:
                ...
                # means a zombie StudioMCP.exe (not ours) still owns port 47171 and
...
async def main():
    print(f"\n{C['cy']}  +--------------------------------------------------+{C['reset']}")
    print(f"{C['cy']}  |                                                  |{C['reset']}")
    print(f"{C['cy']}  |    CoreLua Bridge  v{BRIDGE_VERSION}              |{C['reset']}")
    print(f"{C['cy']}  |    Roblox Studio Agent                            |{C['reset']}")
    print(f"{C['cy']}  |                                                  |{C['reset']}")
    print(f"{C['cy']}  +--------------------------------------------------+{C['reset']}")
    print(f"{C['dim']}  ws://{HOST}:{PORT}{C['reset']}\n")
    ...
    try:
        ...
        if getattr(e, "errno", None) in (98, 10048) or "10048" in str(e):
            ...
            log(f"    Or set a different port before start.bat:  set CL_BRIDGE_PORT=17614", "yl")
            return
        raise
