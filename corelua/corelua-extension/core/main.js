...
// Drives any AI chat site through the CLProvider interface (providers/*.js):
// waits for the model's reply, parses CoreLua commands (CLParse), asks the
...
(() => {
  ...
  const log = (...a) => console.log("[corelua]", ...a);
  ...
  // console (filter "[cl-diag]") or window.__zsDiag (also mirrored onto a hidden
  // DOM node for a main-world inspector). Each entry carries a turn snapshot.
  const CL_DIAG_MAX = 300;
  const _diag = [];
  function diag(event, data) {
    ...
    const e = { t: Date.now(), iso: new Date().toISOString().slice(11, 23), event,
                data: data || null, snap };
    _diag.push(e);
    if (_diag.length > CL_DIAG_MAX) _diag.shift();
    try { console.log("[cl-diag]", e.iso, event, JSON.stringify({ ...data, ...snap })); } catch {}
    try {
      let n = document.getElementById("cl-diag-log");
      if (!n) { n = document.createElement("script"); n.type = "application/json"; n.id = "cl-diag-log"; (document.body || document.documentElement).appendChild(n); }
  ...
  // AI chat sites CoreLua works on. Keep in sync with manifest.json
  ...
  const A = {
    ...
    toolsAt: 0,
    // Wall-clock start of the current agent loop (set in agentLoop, used by the
    // finally block to fire a desktop notification only for sessions long enough
    // to be worth alerting about - short turns would just spam the user).
    loopStart: 0,
  };
  ...
  async function waitForResponse(base) {
    ...
    while (Date.now() - lastActiveAt < TIMEOUT) {
      ...
      const blockActive = CLParse.hasOpenToolBlock(d.reply) && Date.now() - lastChangeAt < 6000;
      ...
      const stuckDone = started && d.reply && Date.now() - lastChangeAt > STABLE_MS &&
        !(gen && CLParse.hasOpenToolBlock(d.reply));
      ...
      const cmdShaped = P.replyUnsettled && (
        CLParse.hasToolSignature(r) ||
        (CLParse.LUA_END_RE.test(r) && !CLParse.LUA_START_RE.test(r)) ||
      ...
      if (CLParse.hasToolSignature(r)) {
        const calls = CLParse.parseToolCalls(r);
        ...
        if (r.includes(CLParse.START_M) || CLParse.LUA_START_RE.test(r)) return { kind: "parse_error", reason: "malformed", raw: r, item: d.item };
        ...
        // while it streamed).
        if (CLParse.hasOpenToolBlock(r)) {
          const saved = CLParse.salvageCutOff(r);
          if (saved) {
        ...
        const nm = CLParse.toolNameFromText(r);
      ...
      if (CLParse.LUA_END_RE.test(r) && !CLParse.LUA_START_RE.test(r) && !r.includes(CLParse.START_M)) {
  ...
  // CL.toolCategory. A custom MCP tool's NAME reveals nothing, so we learn which
  ...
  async function ensureTools(force) {
    ...
    // Browser-side virtual commands that don't go through the bridge/Studio.
    // They are ALWAYS available (even with Roblox offline), so inject them into
    // the cached catalogue regardless of what the bridge returned. We add them
    // only once and skip any name the bridge itself already advertises (it won't,
    // but guard anyway).
    const VIRTUAL_TOOLS = [
      {
        name: "notify_user",
        description: "Show a desktop notification to the user (title + message). Use it to alert the user when a long-running step finishes, or when you need their attention while they may be on another tab.",
        inputSchema: {
          type: "object",
          properties: {
            title: { type: "string", description: "Short title for the notification (max 80 chars). Defaults to 'CoreLua'." },
            message: { type: "string", description: "Body text of the notification (max 280 chars)." },
          },
        },
      },
      {
        name: "get_page_info",
        description: "Return the current browser tab's URL, title, host, origin and pathname. Use it when you need context about the page the user is on (e.g. a doc, a Roblox Creator page) - no parameters needed.",
        inputSchema: { type: "object", properties: {} },
      },
      // ── 3D construction tools (high-level wrappers around execute_luau) ──
      // These let the model build geometry in one call instead of hand-writing
      // long Instance.new chains. They generate robust Luau and run it in Studio.
      {
        name: "build_3d_model",
        description: "Build a named 3D model from a high-level shape recipe and parent it into the game tree. Generates robust Luau (boxes, wedges, cylinders, spheres, corner wedges) and runs it in Studio. Prefer this over hand-written execute_luau for anything with real geometry.",
        inputSchema: {
          type: "object",
          properties: {
            name: { type: "string", description: "Name of the Model to create (parented under Workspace by default)." },
            parent: { type: "string", description: "Dot-path parent, e.g. 'Workspace', 'Workspace.Decorations'. Defaults to 'Workspace'." },
            shapes: {
              type: "array",
              description: "Ordered list of shapes. Each becomes a Part/MeshPart inside the model.",
              items: {
                type: "object",
                properties: {
                  type: { type: "string", description: "Shape kind: 'box' | 'wedge' | 'cylinder' | 'sphere' | 'cornerwedge' | 'ball' | 'block'." },
                  name: { type: "string", description: "Instance name for this shape." },
                  size: { type: "string", description: "Vector3 size as 'x,y,z' (studs). e.g. '4,2,6'." },
                  pos: { type: "string", description: "Position as 'x,y,z' (studs). e.g. '0,5,0'. Optional, defaults to 0,0,0." },
                  rot: { type: "string", description: "Rotation as 'x,y,z' degrees. Optional." },
                  color: { type: "string", description: "BrickColor name or Color3 'r,g,b' (0-255). Optional." },
                  material: { type: "string", description: "Roblox Material enum name, e.g. 'Plastic','Neon','Metal','Glass','Wood'. Optional." },
                  anchor: { type: "boolean", description: "Anchored? Default true." },
                },
              },
            },
            weld: { type: "boolean", description: "If true, weld all shapes together with WeldConstraints so the model is one rigid body. Default false." },
          },
        },
      },
      {
        name: "create_primitives",
        description: "Batch-create primitive Parts (boxes, cylinders, spheres, wedges) with explicit size/position/color/material in a single call. Lighter than build_3d_model when you just need loose parts, not a grouped model.",
        inputSchema: {
          type: "object",
          properties: {
            parent: { type: "string", description: "Dot-path parent, defaults to 'Workspace'." },
            parts: {
              type: "array",
              description: "Each item becomes one Part.",
              items: {
                type: "object",
                properties: {
                  shape: { type: "string", description: "'box' | 'cylinder' | 'sphere' | 'wedge' | 'cornerwedge'." },
                  name: { type: "string" },
                  size: { type: "string", description: "'x,y,z' studs." },
                  pos: { type: "string", description: "'x,y,z' studs." },
                  rot: { type: "string", description: "'x,y,z' degrees. Optional." },
                  color: { type: "string", description: "BrickColor name or 'r,g,b'. Optional." },
                  material: { type: "string", description: "Material enum name. Optional." },
                  anchored: { type: "boolean", description: "Default true." },
                },
              },
            },
          },
      ...
      {
        name: "apply_material",
        description: "Apply material, color, transparency, reflectance to one or many instances at once. Resolves BrickColor names, Color3 and Material enums safely.",
        inputSchema: {
          type: "object",
          properties: {
            targets: { type: "string", description: "Dot-path, or a comma-separated list of dot-paths. All must resolve to BaseParts (or Models whose descendants are BaseParts)." },
            material: { type: "string", description: "Material enum name (e.g. 'Neon','Glass','Metal','Ice','WoodPlanks'). Optional." },
            color: { type: "string", description: "BrickColor name or Color3 'r,g,b' (0-255). Optional." },
            transparency: { type: "number", description: "0 to 1. Optional." },
            reflectance: { type: "number", description: "0 to 1. Optional." },
          },
        },
      },
      {
        name: "group_and_parent",
        description: "Group a set of existing instances into a Model (or Folder), set the PrimaryPart, and re-parent it. Computes a sensible pivot from the children if no primary is given.",
        inputSchema: {
          type: "object",
          properties: {
            model_name: { type: "string", description: "Name of the Model/Folder to create." },
            kind: { type: "string", description: "'Model' (default) or 'Folder'." },
            members: { type: "array", description: "Dot-paths of instances to move into the group.", items: { type: "string" } },
            parent: { type: "string", description: "Where to parent the group. Defaults to 'Workspace'." },
            primary: { type: "string", description: "Dot-path of the PrimaryPart (Model only). Optional." },
          },
        },
      },
      {
        name: "weld_rigid",
        description: "Weld parts together into a single rigid body using WeldConstraints. Provide a root and a list of parts to weld to the root (or weld all descendants of a model to its PrimaryPart).",
        inputSchema: {
          type: "object",
          properties: {
            root: { type: "string", description: "Dot-path of the root BasePart." },
            parts: { type: "array", description: "Dot-paths of BaseParts to weld to root. Optional if mode='model'.", items: { type: "string" } },
            mode: { type: "string", description: "'list' (default, use parts) or 'model' (weld all BasePart descendants of root's model to root)." },
          },
        },
      },
      {
        name: "csg_union",
        description: "Boolean UNION of two or more parts into a single UnionOperation (combines geometry). The first target is the base; the rest are merged into it. Anchors/responsibilities handled safely.",
        inputSchema: {
          type: "object",
          properties: {
            targets: { type: "array", description: "Ordered dot-paths of BaseParts to union. First is the base.", items: { type: "string" } },
            name: { type: "string", description: "Name for the resulting UnionOperation. Optional." },
            parent: { type: "string", description: "Parent for the result. Defaults to the base's parent." },
          },
        },
      },
      {
        name: "csg_subtract",
        description: "Boolean SUBTRACT: cut the first part (base) using the subsequent parts as cutters, producing a UnionOperation with holes. Great for carving doors/windows out of walls.",
        inputSchema: {
          type: "object",
          properties: {
            base: { type: "string", description: "Dot-path of the BasePart to carve." },
            cutters: { type: "array", description: "Dot-paths of BaseParts used as cutters.", items: { type: "string" } },
            name: { type: "string", description: "Name for the resulting UnionOperation. Optional." },
            keep_cutters: { type: "boolean", description: "If true, the cutter parts are kept in the tree (moved aside). Default false (destroyed)." },
          },
        },
      },
      {
        name: "csg_intersect",
        description: "Boolean INTERSECT: keep only the overlapping volume of the given parts, producing a UnionOperation.",
        inputSchema: {
          type: "object",
          properties: {
            targets: { type: "array", description: "Dot-paths of BaseParts to intersect.", items: { type: "string" } },
            name: { type: "string", description: "Name for the resulting UnionOperation. Optional." },
          },
        },
      },
      {
        name: "camera_focus",
        description: "Move the Studio EDIT camera to frame a given instance (or position) so the user can preview the 3D result. Uses the current camera; no play needed.",
        inputSchema: {
          type: "object",
          properties: {
            target: { type: "string", description: "Dot-path of the instance to frame, OR a position 'x,y,z'." },
            distance: { type: "number", description: "Camera distance in studs. Optional, default 12." },
          },
        },
      },
      // ── GUI builder: create rich 2D interfaces in one call ──
      {
        name: "create_gui",
        description: "Build a complete Roblox GUI (ScreenGui + tree of Frames, Buttons, Labels, etc.) from a structured recipe and parent it into StarterGui. Generates robust Luau with UICorner, UIStroke, UIGradient, UIListLayout, UIGridLayout support. Prefer this over hand-written execute_luau for any UI with multiple elements, buttons, navigation bars, shop grids, menus, HUDs, dialogs. Each element can have children (nested frames), decorations (corner, stroke, gradient), and layout constraints.",
        inputSchema: {
          type: "object",
          properties: {
            name: { type: "string", description: "Name of the ScreenGui to create." },
            parent: { type: "string", description: "Dot-path parent. Defaults to 'StarterGui'. Use 'StarterPlayer.StarterPlayerScripts' for screenless UI, or a Player's PlayerGui at runtime." },
            reset_on_spawn: { type: "boolean", description: "ScreenGui.ResetOnSpawn. Default false (keeps the GUI alive across respawns)." },
            display_order: { type: "integer", description: "ScreenGui.DisplayOrder (z-index). Higher = on top. Default 0." },
            ignore_gui_inset: { type: "boolean", description: "ScreenGui.IgnoreGuiInset. If true, the GUI fills the entire screen including the top bar. Default true." },
            elements: {
              type: "array",
              description: "Top-level elements inside the ScreenGui. Each element can recursively have 'children', 'corner', 'stroke', 'gradient', 'layout'.",
        ...
        definitions: {
          element: {
            type: "object",
            properties: {
              class: { type: "string", description: "Roblox GUI class: 'Frame', 'TextButton', 'TextBox', 'TextLabel', 'ImageButton', 'ImageLabel', 'ScrollingFrame', 'VideoFrame', 'ViewportFrame'. Default 'Frame'." },
              name: { type: "string", description: "Instance name." },
              size: { type: "string", description: "UDim2 as 'sx,ox,sy,oy' (scale, offset). e.g. '1,0,0,60' = full width, 60px tall. Or '0,200,0,50' = 200x50px." },
              position: { type: "string", description: "UDim2 as 'sx,ox,sy,oy'. e.g. '0.5,-100,0.5,-25' = centered 200x50. Default '0,0,0,0'." },
              anchor_point: { type: "string", description: "Vector2 anchor as 'x,y' (0-1). e.g. '0.5,0.5' = center. Default '0,0'." },
              background_color: { type: "string", description: "BrickColor name or 'r,g,b' (0-255)." },
              background_transparency: { type: "number", description: "0 = opaque, 1 = invisible. Default 0." },
              border_color: { type: "string", description: "BrickColor or 'r,g,b'. Optional." },
              border_size_pixel: { type: "integer", description: "Border thickness in px. Default 0 (use UIStroke instead)." },
              visible: { type: "boolean", description: "Default true." },
              zindex: { type: "integer", description: "ZIndex within the ScreenGui. Default 1." },
              rotation: { type: "number", description: "Rotation in degrees. Default 0." },
              // Text-specific (TextButton, TextBox, TextLabel)
              text: { type: "string", description: "Text content." },
              text_color: { type: "string", description: "BrickColor or 'r,g,b'. Default white (255,255,255)." },
              text_size: { type: "integer", description: "Font size in px. Default 14." },
              font: { type: "string", description: "Font enum name: 'SourceSans','GothamBold','GothamSemibold','GothamMedium','Gotham','ArialBold','Legacy','SourceSansBold','SourceSansItalic','SourceSansLight','SourceSansSemibold','Montserrat','Code','Highway','Roboto','RobotoCondensed','RobotoMono','AmaticSC','Balthazar','Creepster','DenkOne','Fantasy','IndieFlower','JosefinSans','Jura','Kalam','LuckiestGuy','Merriweather','Nunito','Oswald','PatrickHand','PermanentMarker','Sarpanch','SpecialElite','TitanOne','Toxia','ZillaSlab'. Default 'Gotham'." },
              text_transparency: { type: "number", description: "0-1. Default 0." },
              text_wraps: { type: "boolean", description: "Default false." },
              text_x_alignment: { type: "string", description: "'Left','Center','Right'. Default 'Center'." },
              text_y_alignment: { type: "string", description: "'Top','Center','Bottom'. Default 'Center'." },
              text_scaled: { type: "boolean", description: "Auto-scale text to fit. Default false." },
              // Image-specific (ImageButton, ImageLabel)
              image: { type: "string", description: "Asset ID (rbxassetid://...) or URL." },
              image_color: { type: "string", description: "Tint color 'r,g,b'. Default white." },
              image_transparency: { type: "number", description: "0-1. Default 0." },
              scale_type: { type: "string", description: "'Stretch','Slice','Tile','Fit'. Default 'Stretch'." },
              // ScrollingFrame-specific
              canvas_size: { type: "string", description: "UDim2 for the scrollable area." },
              scroll_bar_thickness: { type: "integer", description: "Default 4." },
              automatic_canvas_size: { type: "string", description: "'None','X','Y','XY'. Default 'None'." },
              // Decorations
              corner: { type: "object", description: "UICorner: rounded corners.", properties: { radius: { type: "string", description: "UDim 's,o' e.g. '0,12' for 12px radius, '0.5,0' for fully round. Default '0,8'." } } },
              stroke: { type: "object", description: "UIStroke: border outline.", properties: { color: { type: "string", description: "'r,g,b' or BrickColor." }, thickness: { type: "number", description: "px. Default 1." }, transparency: { type: "number", description: "0-1. Default 0." }, mode: { type: "string", description: "'Border','Outline','Middle'. Default 'Border'." } } },
              gradient: { type: "object", description: "UIGradient: color gradient fill.", properties: { colors: { type: "array", description: "Array of 'r,g,b' color strings.", items: { type: "string" } }, rotation: { type: "number", description: "Degrees. Default 0 (left to right)." }, offsets: { type: "array", description: "Optional time-keyframes 0-1 for each color.", items: { type: "number" } } } },
              padding: { type: "object", description: "UIPadding: inner padding.", properties: { top: { type: "string", description: "UDim 's,o'." }, bottom: { type: "string" }, left: { type: "string" }, right: { type: "string" } } },
              // Layout
              layout: { type: "object", description: "UIListLayout or UIGridLayout for auto-arranging children.", properties: { type: { type: "string", description: "'List' or 'Grid'." }, direction: { type: "string", description: "For List: 'Horizontal' or 'Vertical'. Default 'Vertical'." }, padding: { type: "string", description: "UDim 's,o' gap between items. Default '0,4'." }, horizontal_alignment: { type: "string", description: "'Left','Center','Right'. For List." }, vertical_alignment: { type: "string", description: "'Top','Center','Bottom'. For List." }, sort_order: { type: "string", description: "'LayoutOrder','Name'. Default 'LayoutOrder'." }, cell_size: { type: "string", description: "UDim2 for Grid. e.g. '0,100,0,100'." }, cell_padding: { type: "string", description: "UDim2 for Grid. e.g. '0,8,0,8'." } } },
              aspect_ratio: { type: "object", description: "UIAspectRatioConstraint: lock width/height ratio.", properties: { ratio: { type: "number", description: "width/height. e.g. 1.5." }, dominance: { type: "string", description: "'Width','Height'. Default 'Width'." } } },
              // Children
              children: { type: "array", description: "Nested child elements (same structure).", items: { "$ref": "#/definitions/element" } },
              layout_order: { type: "integer", description: "Sort order when parent has a layout. Default 0." },
            },
          },
        },
      },
      // ── Instance management tools ──
      {
        ...
        description: "Delete (Destroy) one or more instances by dot-path. Destructive: the instance and ALL its descendants are permanently removed from the game tree. Use with care - prefer duplicate_instance for a safe copy first. Returns the count of deleted vs not-found instances.",
        inputSchema: {
          type: "object",
          properties: {
            targets: { type: "array", description: "Dot-paths of instances to delete, e.g. ['Workspace.OldPart', 'ReplicatedStorage.UnusedModule'].", items: { type: "string" } },
            confirm: { type: "boolean", description: "Must be true to proceed. Safety guard so the AI does not delete by accident." },
          },
      ...
      },
      {
        ...
        description: "Duplicate (Clone) an existing instance by dot-path. The clone is parented to the same parent as the original (or a custom parent) and optionally renamed. Great for copying parts, models, or GUI templates. Returns the full path of the clone.",
        inputSchema: {
          type: "object",
          properties: {
            source: { type: "string", description: "Dot-path of the instance to clone, e.g. 'Workspace.Building.Floor1'." },
            new_name: { type: "string", description: "Name for the clone. Optional, defaults to the original's name." },
            parent: { type: "string", description: "Dot-path parent for the clone. Optional, defaults to the original's parent." },
            position_offset: { type: "string", description: "Offset to apply to the clone's position if it's a BasePart or Model, as 'x,y,z' in studs. Optional, default '0,0,0'." },
          },
          required: ["source"],
        },
      },
      {
        name: "set_property",
        description: "Set one or more properties on an existing instance by dot-path. Handles common Roblox property types automatically: Vector3 (CFrame.Position, Size), CFrame, Color3, UDim2, UDim, NumberRange, BrickColor, Enum values, booleans, numbers, and strings. For nested properties use dot notation (e.g. 'PrimaryPart.CFrame'). Prefer this over execute_luau for simple property changes.",
        inputSchema: {
          type: "object",
          properties: {
            target: { type: "string", description: "Dot-path of the instance, e.g. 'Workspace.Baseplate'." },
            properties: { type: "object", description: "Key-value map of property name -> value string. Values are parsed by type: 'x,y,z' for Vector3, 'sx,ox,sy,oy' for UDim2, 'r,g,b' for Color3, 'name' for enums, true/false for booleans.", additionalProperties: { type: "string" } },
          },
      ...
      {
        name: "move_instance",
        description: "Reparent and/or reposition an existing instance. Can change its Parent (move it in the hierarchy tree) and/or set its CFrame/Position. Use this to relocate parts, reorganize the game tree, or move models. Returns the new full path.",
        inputSchema: {
          type: "object",
          properties: {
            target: { type: "string", description: "Dot-path of the instance to move." },
            new_parent: { type: "string", description: "Dot-path of the new parent. Optional - if omitted, the instance keeps its current parent." },
            position: { type: "string", description: "New world position as 'x,y,z' studs. Sets CFrame.Position for BaseParts or WorldPivot for Models. Optional." },
            rotation: { type: "string", description: "Rotation in degrees as 'rx,ry,rz'. Applied as CFrame.Angles. Optional." },
            offset: { type: "string", description: "Relative offset from current position as 'x,y,z' studs. Optional, alternative to absolute position." },
          },
          required: ["target"],
        },
      },
      {
        name: "insert_from_toolbox",
        description: "Insert a model from the Roblox Creator Store (formerly Toolbox) by asset ID. Downloads the asset and parents it into the game tree. Use this to add pre-made models, meshes, decals, sounds, or packages from the Creator Store without leaving the AI workflow.",
        inputSchema: {
          type: "object",
          properties: {
            asset_id: { type: "integer", description: "The numeric Roblox asset ID to insert (from the Creator Store URL, e.g. 1234567890)." },
            parent: { type: "string", description: "Dot-path parent for the inserted asset. Defaults to 'Workspace'." },
            name: { type: "string", description: "Name to give the inserted instance. Optional." },
          },
          required: ["asset_id"],
        },
      },
      {
        ...
        description: "Create a KeyframeSequence animation for a Humanoid/Rig from a simple keyframe recipe. Generates the KeyframeSequence with Keyframes and Poses, saves it as an Animation asset reference, and returns the animation ID. Supports position and rotation keyframes per body part (Motor6D names). Use this for walk cycles, idle animations, attack animations, etc.",
        inputSchema: {
          type: "object",
          properties: {
            name: { type: "string", description: "Name for the KeyframeSequence, e.g. 'WalkCycle' or 'Attack1'." },
            parent: { type: "string", description: "Where to store the Animation object. Defaults to 'ReplicatedStorage.Animations' (auto-created if missing)." },
            loop: { type: "boolean", description: "AnimationId loop property. Default false." },
            priority: { type: "string", description: "Animation priority enum: 'Idle','Movement','Action','Action2','Action3','Action4','Core','None'. Default 'Action'." },
            keyframes: { type: "array", description: "List of keyframes. Each has a time (seconds) and poses (per-body-part transforms).", items: { type: "object", properties: { time: { type: "number", description: "Time in seconds for this keyframe." }, poses: { type: "array", description: "Body part poses at this keyframe.", items: { type: "object", properties: { part: { type: "string", description: "Motor6D name / body part, e.g. 'HumanoidRootPart','LeftUpperArm','Head'." }, position: { type: "string", description: "Position offset as 'x,y,z' studs relative to default. Optional." }, rotation: { type: "string", description: "Rotation in degrees as 'rx,ry,rz'. Optional." } } } } } } },
          },
          required: ["name", "keyframes"],
        },
      },
      {
        name: "create_remote_event",
        description: "Create RemoteEvents and/or RemoteFunctions in one call. Parents them to the specified container (auto-creates a Folder if needed). Use this to set up client-server communication channels quickly. Each remote has a name and type ('RemoteEvent' or 'RemoteFunction').",
        inputSchema: {
          type: "object",
          properties: {
            parent: { type: "string", description: "Dot-path container for the remotes. Defaults to 'ReplicatedStorage.Remotes' (auto-created Folder)." },
            remotes: { type: "array", description: "List of remotes to create.", items: { type: "object", properties: { name: { type: "string", description: "Name of the RemoteEvent/RemoteFunction." }, type: { type: "string", description: "'RemoteEvent' (default) or 'RemoteFunction'." } } } },
          },
          required: ["remotes"],
        },
      },
      {
        name: "create_gui_template",
        description: "Create a complete prefab GUI from a named template. Available templates: 'shop' (item shop with grid layout, coin display, buy buttons), 'hud' (health bar, coins, level, minimap placeholder), 'settings' (settings menu with sliders/toggles and a close button), 'dialog' (NPC dialog box with name, text, and next/close buttons), 'loading' (loading screen with spinner, progress bar, and status text). Optional 'overrides' object customizes colors, title text, etc. The GUI is created in StarterGui and ready to wire to scripts.",
        inputSchema: {
          type: "object",
          properties: {
            template: { type: "string", enum: ["shop", "hud", "settings", "dialog", "loading"], description: "The prefab template to use." },
            name: { type: "string", description: "Name for the ScreenGui (default: based on template)." },
            parent: { type: "string", description: "Parent path (default: StarterGui)." },
            overrides: { type: "object", description: "Optional overrides: {title, accent_color, bg_color, text_color, dialog_text, status_text, health_color} to customize the template." },
            reset_on_spawn: { type: "boolean", description: "Reset GUI on respawn (default: false)." },
            ignore_gui_inset: { type: "boolean", description: "Ignore the top bar inset (default: true)." }
          },
          required: ["template"]
        }
      },
      // ── Coding tools (Luau expert helpers) ──
      {
        name: "code_gen_luau",
        description: "Generate a complete, production-ready Luau ModuleScript/Script/LocalScript from a short spec. Produces clean, idiomatic, typed (where useful) Roblox code with error handling, and creates the script via multi_edit. Prefer this for non-trivial systems (data, remotes, state machines, services).",
        inputSchema: {
          type: "object",
          properties: {
            path: { type: "string", description: "Dot-path where the script should live, e.g. 'ServerScriptService.CombatService' or 'ReplicatedStorage.Shared.WeaponConfig'." },
            class: { type: "string", description: "'ModuleScript' (default), 'Script', or 'LocalScript'." },
            spec: { type: "string", description: "What the script must do, in plain language. Be specific about inputs, outputs, services, events, and edge cases." },
            style: { type: "string", description: "Optional style hints: 'typed' (use type annotations), 'oop' (classes), 'functional', 'minimal'. Default 'typed'." },
          },
        },
      },
      {
        name: "code_refactor",
        description: "Refactor an existing Luau script for readability, performance and Roblox best practices (avoid Instance.new without parent, use task.* over wait, type annotations, early returns, no global state). Reads the script, rewrites it, writes it back. Returns a short summary of changes.",
        inputSchema: {
          type: "object",
          properties: {
            path: { type: "string", description: "Dot-path of the script to refactor." },
            goals: { type: "string", description: "Optional extra goals, e.g. 'extract the cooldown logic into its own function', 'add type annotations'." },
          },
        },
      },
      {
        name: "code_review",
        description: "Review a Luau script and return a structured report: bugs, anti-patterns, performance issues, Roblox-specific gotchas (yielding on the main thread, unanchored parts, RemoteEvent security), and concrete fix suggestions. Does NOT modify the script.",
        inputSchema: {
          type: "object",
          properties: {
            path: { type: "string", description: "Dot-path of the script to review." },
            focus: { type: "string", description: "Optional focus: 'security', 'performance', 'bugs', 'style'. Default: all." },
          },
        },
      },
      {
        name: "code_add_tests",
        description: "Generate a Luau test harness for a ModuleScript (using a tiny built-in assert runner, no external framework needed) and create it next to the module. Helps catch regressions on data/logic modules.",
        inputSchema: {
          type: "object",
          properties: {
            module_path: { type: "string", description: "Dot-path of the ModuleScript to test." },
            cases: { type: "string", description: "Optional description of specific cases to cover. If omitted, reasonable defaults are inferred from the module." },
          },
        },
      },
    ];
    const existing = new Set(A.toolList.map((t) => t.name));
    for (const vt of VIRTUAL_TOOLS) {
      if (!existing.has(vt.name)) A.toolList.push(vt);
    }
    A.toolNames = new Set(A.toolList.map((t) => t.name));
    A.toolsAt = Date.now();
    return A.toolList;
  }
  ...
  async function runTool(call) {
    ...
    if (!name) return CL.FEEDBACK.parseError("malformed");
    ...
    // Virtual command: list the MCP server(s) CoreLua is currently connected
    ...
    if (name === "list_commands" || name === "list_tools") {
      ...
      const lines = scoped.map((t) => {
        ...
        const note = CL.TOOL_NOTES[bareToolName(t.name)];
        const noteStr = note ? `\n    ⚠ ${note}` : "";
        return `${t.name}: ${(t.description || "").split("\n")[0]}${paramLines ? "\n" + paramLines : ""}${noteStr}`;
      });
      return `Output of '${name}':\n${requested} commands (${scoped.length}):\n\n${lines.join("\n\n")}`;
    }
    // Virtual command: notify_user — the AI fires a desktop notification itself
    // (in addition to the automatic one agentLoop fires on session end). Routes
    // through the background service worker (chrome.notifications.create is only
    // allowed there). No args are strictly required; title/message default.
    if (name === "notify_user") {
      const title = (args.title || "CoreLua").toString().slice(0, 80);
      const message = (args.message || "The agent has an update for you.").toString().slice(0, 280);
      const r = await bg({ type: "notify", title, message });
      if (r && r.ok) {
        return `Output of 'notify_user':\nDesktop notification sent (title: "${title}").`;
      }
      return `Output of 'notify_user':\nWARNING: the desktop notification could not be sent (${(r && r.error) || "unknown error"}). The user may not have granted the notifications permission, or the service worker was asleep. Continue normally - this is not blocking.`;
    }
    // Virtual command: get_page_info — returns the current tab's URL and title.
    // Useful when the AI needs page context (e.g. the user pointed it at a doc
    // or a Roblox Creator page open in this tab). Runs entirely browser-side; no
    // bridge / Studio round-trip.
    if (name === "get_page_info") {
      ...
      return `Output of 'get_page_info':\nCurrent tab:\n  url: ${info.url}\n  title: ${info.title}\n  host: ${info.host}\n  origin: ${info.origin}\n  pathname: ${info.pathname}`;
    }

    // ─────────────────────────────────────────────────────────────────────
    // 3D + CODING META-TOOLS
    // High-level wrappers that generate robust Luau and run it in Studio via
    // the real execute_luau channel (reentrant runTool). They keep the model
    // from hand-writing long, error-prone Instance.new chains and let it build
    // geometry / refactor code in ONE call.
    // ─────────────────────────────────────────────────────────────────────

    // Run Luau in Studio (Edit datamodel) and return the textual result string
    // the way the agent loop expects it. Reentrant: calls runTool itself.
    async function runLua(code) {
      const res = await runTool({ tool: "execute_luau", arguments: { code, datamodel_type: "Edit" } });
      return typeof res === "string" ? res : String(res);
    }
    // Tiny safe JSON-encode for embedding values into generated Luau.
    const jq = (v) => JSON.stringify(v);
    // Resolve a comma list / single dot-path string into an array of paths.
    const splitPaths = (s) => (s || "").split(",").map((x) => x.trim()).filter(Boolean);
    // Parse "x,y,z" -> "Vector3.new(x, y, z)" Luau; tolerant of spaces.
    const v3 = (s, fallback) => {
      if (!s || !String(s).trim()) return fallback || "Vector3.new(0, 0, 0)";
      const parts = String(s).split(",").map((x) => parseFloat(x.trim()));
      if (parts.length !== 3 || parts.some((n) => isNaN(n))) throw new Error(`Bad Vector3 '${s}' (expected 'x,y,z')`);
      return `Vector3.new(${parts[0]}, ${parts[1]}, ${parts[2]})`;
    };
    const degRot = (s) => {
      if (!s || !String(s).trim()) return "CFrame.new()";
      const p = String(s).split(",").map((x) => parseFloat(x.trim()));
      if (p.length !== 3 || p.some((n) => isNaN(n))) throw new Error(`Bad rotation '${s}' (expected 'x,y,z' degrees)`);
      const [x, y, z] = p.map((d) => (d * Math.PI) / 180);
      return `CFrame.Angles(${x}, ${y}, ${z})`;
    };
    // Generate Luau that resolves a color string (BrickColor name OR "r,g,b").
    const colorExpr = (s) => {
      if (!s || !String(s).trim()) return null;
      const str = String(s).trim();
      if (/^\d+(\.\d+)?\s*,\s*\d+(\.\d+)?\s*,\s*\d+(\.\d+)?$/.test(str)) {
        const [r, g, b] = str.split(",").map((x) => parseFloat(x.trim()));
        return `Color3.fromRGB(${Math.round(r)}, ${Math.round(g)}, ${Math.round(b)})`;
      }
      // Treat as BrickColor name.
      return `(BrickColor.new(${jq(str)}).Color)`;
    };
    // Resolve a Material enum name safely.
    const materialExpr = (s) => (s && String(s).trim() ? `Enum.Material.${String(s).trim()}` : null);

    // ── build_3d_model ──
    if (name === "build_3d_model") {
      const modelName = (args.name || "GeneratedModel").toString();
      const parentPath = (args.parent || "Workspace").toString();
      const shapes = Array.isArray(args.shapes) ? args.shapes : [];
      if (!shapes.length) return `ERROR: build_3d_model needs a non-empty 'shapes' array.`;
      const wantWeld = args.weld !== false;
      let code = `-- CoreLua build_3d_model: ${modelName}\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local parent = safePath(${jq(parentPath)}) or game:GetService("Workspace")\n`;
      code += `local model = Instance.new("Model")\nmodel.Name = ${jq(modelName)}\nmodel.Parent = parent\n`;
      code += `local created = {}\n`;
      code += `local function resolveColor(c) if not c then return nil end if string.find(c, ",") then local r,g,b = string.match(c, "(%d+%.?%d*)%s*,%s*(%d+%.?%d*)%s*,%s*(%d+%.?%d*)") return Color3.fromRGB(tonumber(r), tonumber(g), tonumber(b)) else return BrickColor.new(c).Color end end\n`;
      for (let i = 0; i < shapes.length; i++) {
        const s = shapes[i] || {};
        const sName = (s.name || `Shape${i + 1}`).toString();
        const kind = (s.type || "box").toString().toLowerCase();
        let className = "Part";
        if (kind === "wedge") className = "WedgePart";
        else if (kind === "cornerwedge") className = "CornerWedgePart";
        code += `do\n`;
        code += `  local p = Instance.new(${jq(className)})\n`;
        code += `  p.Name = ${jq(sName)}\n`;
        try {
          code += `  p.Size = ${v3(s.size, "Vector3.new(1, 1, 1)")}\n`;
          if (s.pos != null) code += `  p.Position = ${v3(s.pos)}\n`;
          if (s.rot != null) code += `  p.CFrame = p.CFrame * ${degRot(s.rot)}\n`;
          const col = colorExpr(s.color);
          if (col) code += `  p.Color = ${col}\n`;
          const mat = materialExpr(s.material);
          if (mat) code += `  p.Material = ${mat}\n`;
          code += `  p.Anchored = ${s.anchor === false ? "false" : "true"}\n`;
          code += `  p.Parent = model\n`;
          code += `  created[#created + 1] = p\n`;
          code += `end\n`;
        } catch (e) {
          return `ERROR: build_3d_model shape #${i + 1} (${sName}): ${e.message}`;
        }
      }
      if (wantWeld && shapes.length > 1) {
        code += `-- weld all shapes into one rigid body (root = first)\n`;
        code += `local root = created[1]\n`;
        code += `for i = 2, #created do local w = Instance.new("WeldConstraint") w.Name = "RigidWeld" w.Part0 = root w.Part1 = created[i] w.Parent = root end\n`;
        code += `if root then model.PrimaryPart = root end\n`;
      }
      code += `return {ok = true, model = model:GetFullName(), parts = #created, welded = ${wantWeld && shapes.length > 1 ? "true" : "false"}}\n`;
      const out = await runLua(code);
      return `Output of 'build_3d_model':\nBuilt model "${modelName}" under ${parentPath}.\n${out}`;
    }

    // ── create_primitives ──
    if (name === "create_primitives") {
      const parentPath = (args.parent || "Workspace").toString();
      const parts = Array.isArray(args.parts) ? args.parts : [];
      if (!parts.length) return `ERROR: create_primitives needs a non-empty 'parts' array.`;
      let code = `-- CoreLua create_primitives\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local parent = safePath(${jq(parentPath)}) or game:GetService("Workspace")\n`;
      code += `local function resolveColor(c) if not c then return nil end if string.find(c, ",") then local r,g,b = string.match(c, "(%d+%.?%d*)%s*,%s*(%d+%.?%d*)%s*,%s*(%d+%.?%d*)") return Color3.fromRGB(tonumber(r), tonumber(g), tonumber(b)) else return BrickColor.new(c).Color end end\n`;
      code += `local created = {}\n`;
      for (let i = 0; i < parts.length; i++) {
        const p = parts[i] || {};
        const shape = (p.shape || "box").toString().toLowerCase();
        let className = "Part";
        if (shape === "cylinder") { className = "Part"; }
        else if (shape === "sphere" || shape === "ball") { className = "Part"; }
        else if (shape === "wedge") className = "WedgePart";
        else if (shape === "cornerwedge") className = "CornerWedgePart";
        code += `do\n`;
        code += `  local part = Instance.new(${jq(className)})\n`;
        code += `  part.Name = ${jq((p.name || `Part${i + 1}`).toString())}\n`;
        try {
          if (shape === "cylinder") code += `  part.Shape = Enum.PartType.Cylinder\n`;
          if (shape === "sphere" || shape === "ball") code += `  part.Shape = Enum.PartType.Ball\n`;
          code += `  part.Size = ${v3(p.size, "Vector3.new(1, 1, 1)")}\n`;
          if (p.pos != null) code += `  part.Position = ${v3(p.pos)}\n`;
          if (p.rot != null) code += `  part.CFrame = part.CFrame * ${degRot(p.rot)}\n`;
          const col = colorExpr(p.color);
          if (col) code += `  part.Color = ${col}\n`;
          const mat = materialExpr(p.material);
          if (mat) code += `  part.Material = ${mat}\n`;
          code += `  part.Anchored = ${p.anchored === false ? "false" : "true"}\n`;
          code += `  part.Parent = parent\n`;
          code += `  created[#created + 1] = part\n`;
          code += `end\n`;
        } catch (e) {
          return `ERROR: create_primitives part #${i + 1}: ${e.message}`;
        }
      }
      code += `return {ok = true, created = #created, parent = parent:GetFullName()}\n`;
      const out = await runLua(code);
      return `Output of 'create_primitives':\nCreated ${parts.length} part(s) under ${parentPath}.\n${out}`;
    }

    // ── apply_material ──
    if (name === "apply_material") {
      const targets = splitPaths(args.targets);
      if (!targets.length) return `ERROR: apply_material needs 'targets' (dot-path or comma list).`;
      const mat = args.material ? materialExpr(args.material) : null;
      const col = colorExpr(args.color);
      const transparency = args.transparency != null ? Number(args.transparency) : null;
      const reflectance = args.reflectance != null ? Number(args.reflectance) : null;
      let code = `-- CoreLua apply_material\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local function resolveColor(c) if not c then return nil end if string.find(c, ",") then local r,g,b = string.match(c, "(%d+%.?%d*)%s*,%s*(%d+%.?%d*)%s*,%s*(%d+%.?%d*)") return Color3.fromRGB(tonumber(r), tonumber(g), tonumber(b)) else return BrickColor.new(c).Color end end\n`;
      code += `local paths = ${jq(targets)}\n`;
      code += `local touched, skipped = 0, 0\n`;
      code += `local function applyTo(part) if not part or not part:IsA("BasePart") then return end touched = touched + 1`;
      if (mat) code += ` part.Material = ${mat}`;
      if (col) code += ` part.Color = resolveColor(${jq(args.color)})`;
      if (transparency != null && !isNaN(transparency)) code += ` part.Transparency = ${transparency}`;
      if (reflectance != null && !isNaN(reflectance)) code += ` part.Reflectance = ${reflectance}`;
      code += ` end\n`;
      code += `for _, p in ipairs(paths) do local inst = safePath(p) if not inst then skipped = skipped + 1 else if inst:IsA("BasePart") then applyTo(inst) else for _, d in ipairs(inst:GetDescendants()) do applyTo(d) end end end end\n`;
      code += `return {ok = true, touched = touched, skipped = skipped}\n`;
      const out = await runLua(code);
      return `Output of 'apply_material':\n${out}`;
    }

    // ── group_and_parent ──
    if (name === "group_and_parent") {
      const members = Array.isArray(args.members) ? args.members.map(String) : [];
      if (!members.length) return `ERROR: group_and_parent needs a non-empty 'members' array.`;
      const kind = (args.kind || "Model").toString();
      const modelName = (args.model_name || "Group").toString();
      const parentPath = (args.parent || "Workspace").toString();
      const primary = args.primary ? String(args.primary) : null;
      let code = `-- CoreLua group_and_parent\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local parent = safePath(${jq(parentPath)}) or game:GetService("Workspace")\n`;
      code += `local group = Instance.new(${jq(kind)})\ngroup.Name = ${jq(modelName)}\ngroup.Parent = parent\n`;
      code += `local moved, missing = 0, 0\n`;
      code += `for _, p in ipairs(${jq(members)}) do local inst = safePath(p) if inst then inst.Parent = group moved = moved + 1 else missing = missing + 1 end end\n`;
      if (kind === "Model") {
        if (primary) {
          code += `local pp = safePath(${jq(primary)}) if pp and pp:IsA("BasePart") then group.PrimaryPart = pp end\n`;
        } else {
          code += `local first = nil for _, d in ipairs(group:GetDescendants()) do if d:IsA("BasePart") then first = d break end end if first then group.PrimaryPart = first end\n`;
        }
        code += `if group.PrimaryPart then group.WorldPivot = group.PrimaryPart.CFrame end\n`;
      }
      code += `return {ok = true, group = group:GetFullName(), moved = moved, missing = missing}\n`;
      const out = await runLua(code);
      return `Output of 'group_and_parent':\n${out}`;
    }

    // ── weld_rigid ──
    if (name === "weld_rigid") {
      const rootPath = (args.root || "").toString();
      if (!rootPath) return `ERROR: weld_rigid needs 'root' (dot-path of the root BasePart).`;
      const mode = (args.mode || "list").toString();
      const parts = Array.isArray(args.parts) ? args.parts.map(String) : [];
      let code = `-- CoreLua weld_rigid\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local root = safePath(${jq(rootPath)})\n`;
      code += `if not root or not root:IsA("BasePart") then return {ok = false, error = "root not found or not a BasePart"} end\n`;
      code += `local welds = 0\n`;
      if (mode === "model") {
        code += `local model = root:FindFirstAncestorOfClass("Model")\nif model then for _, d in ipairs(model:GetDescendants()) do if d:IsA("BasePart") and d ~= root then local w = Instance.new("WeldConstraint") w.Name = "RigidWeld" w.Part0 = root w.Part1 = d w.Parent = root welds = welds + 1 end end end\n`;
      } else {
        if (!parts.length) return `ERROR: weld_rigid in 'list' mode needs a non-empty 'parts' array.`;
        code += `for _, p in ipairs(${jq(parts)}) do local part = safePath(p) if part and part:IsA("BasePart") and part ~= root then local w = Instance.new("WeldConstraint") w.Name = "RigidWeld" w.Part0 = root w.Part1 = part w.Parent = root welds = welds + 1 end end\n`;
      }
      code += `return {ok = true, root = root:GetFullName(), welds = welds}\n`;
      const out = await runLua(code);
      return `Output of 'weld_rigid':\n${out}`;
    }

    // ── CSG helpers (union / subtract / intersect) ──
    async function csgOp(op) {
      // op: { opName, kind: "UnionAsync"|"SubtractAsync"|"IntersectAsync", base, others, name, parent, keepCutters }
      let code = `-- CoreLua ${op.opName}\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local function resolve(path) local p = safePath(path) if not p or not p:IsA("BasePart") then error("Not a BasePart: " .. tostring(path)) end return p end\n`;
      code += `local base = resolve(${jq(op.base)})\n`;
      code += `local others = {}\n`;
      for (const p of op.others) code += `others[#others + 1] = resolve(${jq(String(p))})\n`;
      code += `local result = base\n`;
      if (op.kind === "SubtractAsync") {
        code += `for i = 1, #others do result = result:SubtractAsync(others[i], Enum.CollisionFidelity.Default) end\n`;
      } else if (op.kind === "UnionAsync") {
        code += `for i = 1, #others do result = result:UnionAsync(others[i], Enum.CollisionFidelity.Default) end\n`;
      } else {
        code += `for i = 1, #others do result = result:IntersectAsync(others[i], Enum.CollisionFidelity.Default) end\n`;
      }
      code += `result.Name = ${jq(op.name || "CSGResult")}\n`;
      code += `result.Parent = ${op.parent ? `safePath(${jq(op.parent)}) or game:GetService("Workspace")` : `base.Parent`}\n`;
      if (op.kind !== "SubtractAsync") {
        // base and others are consumed by union/intersect: destroy them.
        code += `if base and base.Parent then base:Destroy() end\nfor _, o in ipairs(others) do if o and o.Parent then o:Destroy() end end\n`;
      } else {
        code += `-- subtract: destroy the base (it's replaced) and optionally the cutters\nif base and base.Parent then base:Destroy() end\n`;
        if (!op.keepCutters) {
          code += `for _, o in ipairs(others) do if o and o.Parent then o:Destroy() end end\n`;
        } else {
          code += `for _, o in ipairs(others) do o.Parent = game:GetService("Workspace") end\n`;
        }
      }
      code += `return {ok = true, result = result:GetFullName()}\n`;
      return runLua(code);
    }

    if (name === "csg_union") {
      const targets = Array.isArray(args.targets) ? args.targets.map(String) : [];
      if (targets.length < 2) return `ERROR: csg_union needs at least 2 'targets' (base + parts to merge).`;
      const out = await csgOp({ opName: "csg_union", kind: "UnionAsync", base: targets[0], others: targets.slice(1), name: args.name, parent: args.parent });
      return `Output of 'csg_union':\n${out}`;
    }
    if (name === "csg_subtract") {
      const base = (args.base || "").toString();
      const cutters = Array.isArray(args.cutters) ? args.cutters.map(String) : [];
      if (!base || !cutters.length) return `ERROR: csg_subtract needs 'base' and a non-empty 'cutters' array.`;
      const out = await csgOp({ opName: "csg_subtract", kind: "SubtractAsync", base, others: cutters, name: args.name, keepCutters: !!args.keep_cutters });
      return `Output of 'csg_subtract':\n${out}`;
    }
    if (name === "csg_intersect") {
      const targets = Array.isArray(args.targets) ? args.targets.map(String) : [];
      if (targets.length < 2) return `ERROR: csg_intersect needs at least 2 'targets'.`;
      const out = await csgOp({ opName: "csg_intersect", kind: "IntersectAsync", base: targets[0], others: targets.slice(1), name: args.name, parent: args.parent });
      return `Output of 'csg_intersect':\n${out}`;
    }

    // ── camera_focus ──
    if (name === "camera_focus") {
      const target = (args.target || "").toString();
      if (!target) return `ERROR: camera_focus needs 'target' (dot-path or 'x,y,z').`;
      const distance = args.distance != null ? Number(args.distance) : 12;
      let code = `-- CoreLua camera_focus\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local v = game:GetService("Workspace").CurrentCamera\nif not v then return {ok = false, error = "no current camera"} end\n`;
      code += `local focus\n`;
      if (/^[-\d.\s,]+$/.test(target) && target.includes(",")) {
        const p = target.split(",").map((x) => parseFloat(x.trim()));
        code += `focus = CFrame.new(Vector3.new(${p[0] || 0}, ${p[1] || 0}, ${p[2] || 0}))\n`;
      } else {
        code += `local inst = safePath(${jq(target)})\nif not inst then return {ok = false, error = "target not found"} end\n`;
        code += `local cf = inst:IsA("BasePart") and inst.CFrame or (inst:IsA("Model") and inst:GetPivot()) or CFrame.new(inst.Position or Vector3.new())\n`;
        code += `focus = cf\n`;
      }
      code += `v.CFrame = focus * CFrame.new(0, ${distance * 0.6}, ${distance}) * CFrame.Angles(math.rad(-20), 0, 0)\n`;
      code += `v.Focus = focus\nreturn {ok = true}\n`;
      const out = await runLua(code);
      return `Output of 'camera_focus':\n${out}`;
    }

    // ── create_gui: build a Roblox ScreenGui from a structured recipe ──
    // Generates Luau that creates a ScreenGui and a tree of GUI elements with
    // decorations (UICorner, UIStroke, UIGradient), layouts (UIListLayout,
    // UIGridLayout), padding, and aspect-ratio constraints.
    function udim2(s, fallback) {
      if (!s || !String(s).trim()) return fallback || "UDim2.new(0, 0, 0, 0)";
      const parts = String(s).split(",").map((x) => parseFloat(x.trim()));
      if (parts.length !== 4 || parts.some((n) => isNaN(n)))
        throw new Error(`Bad UDim2 '${s}' (expected 'sx,ox,sy,oy')`);
      return `UDim2.new(${parts[0]}, ${parts[1]}, ${parts[2]}, ${parts[3]})`;
    }
    function udim(s, fallback) {
      if (!s || !String(s).trim()) return fallback || "UDim.new(0, 0)";
      const parts = String(s).split(",").map((x) => parseFloat(x.trim()));
      if (parts.length !== 2 || parts.some((n) => isNaN(n)))
        throw new Error(`Bad UDim '${s}' (expected 's,o')`);
      return `UDim.new(${parts[0]}, ${parts[1]})`;
    }
    function vector2(s, fallback) {
      if (!s || !String(s).trim()) return fallback || "Vector2.new(0, 0)";
      const parts = String(s).split(",").map((x) => parseFloat(x.trim()));
      if (parts.length !== 2 || parts.some((n) => isNaN(n)))
        throw new Error(`Bad Vector2 '${s}' (expected 'x,y')`);
      return `Vector2.new(${parts[0]}, ${parts[1]})`;
    }
    // Recursively generate Luau for a GUI element and its children.
    // parentVar is the Luau variable name of the parent instance.
    // depth is used for indentation and unique variable naming.
    let guiElemCount = 0;
    function guiElementLuau(elem, parentVar, depth) {
      const cls = (elem.class || "Frame").toString();
      const name = (elem.name || `${cls}${guiElemCount}`).toString();
      const varName = `e${guiElemCount++}`;
      let indent = "  ".repeat(depth);
      let lines = [];
      lines.push(`${indent}do`);
      lines.push(`${indent}  local ${varName} = Instance.new(${jq(cls)})`);
      lines.push(`${indent}  ${varName}.Name = ${jq(name)}`);
      lines.push(`${indent}  ${varName}.Parent = ${parentVar}`);
      // Core properties
      if (elem.size) lines.push(`${indent}  ${varName}.Size = ${udim2(elem.size, "UDim2.new(1, 0, 1, 0)")}`);
      if (elem.position) lines.push(`${indent}  ${varName}.Position = ${udim2(elem.position)}`);
      if (elem.anchor_point) lines.push(`${indent}  ${varName}.AnchorPoint = ${vector2(elem.anchor_point)}`);
      const bg = colorExpr(elem.background_color);
      if (bg) lines.push(`${indent}  ${varName}.BackgroundColor3 = ${bg}`);
      if (elem.background_transparency != null) lines.push(`${indent}  ${varName}.BackgroundTransparency = ${Number(elem.background_transparency)}`);
      const bc = colorExpr(elem.border_color);
      if (bc) lines.push(`${indent}  ${varName}.BorderColor3 = ${bc}`);
      if (elem.border_size_pixel != null) lines.push(`${indent}  ${varName}.BorderSizePixel = ${Number(elem.border_size_pixel)}`);
      if (elem.visible === false) lines.push(`${indent}  ${varName}.Visible = false`);
      if (elem.zindex != null) lines.push(`${indent}  ${varName}.ZIndex = ${Number(elem.zindex)}`);
      if (elem.rotation != null) lines.push(`${indent}  ${varName}.Rotation = ${Number(elem.rotation)}`);
      if (elem.layout_order != null) lines.push(`${indent}  ${varName}.LayoutOrder = ${Number(elem.layout_order)}`);
      // Text properties
      if (elem.text != null) {
        lines.push(`${indent}  ${varName}.Text = ${jq(elem.text)}`);
        const tc = colorExpr(elem.text_color || "255,255,255");
        if (tc) lines.push(`${indent}  ${varName}.TextColor3 = ${tc}`);
        if (elem.text_size != null) lines.push(`${indent}  ${varName}.TextSize = ${Number(elem.text_size)}`);
        if (elem.font) lines.push(`${indent}  ${varName}.Font = Enum.Font.${elem.font}`);
        if (elem.text_transparency != null) lines.push(`${indent}  ${varName}.TextTransparency = ${Number(elem.text_transparency)}`);
        if (elem.text_wraps) lines.push(`${indent}  ${varName}.TextWrapped = true`);
        if (elem.text_x_alignment) lines.push(`${indent}  ${varName}.TextXAlignment = Enum.TextXAlignment.${elem.text_x_alignment}`);
        if (elem.text_y_alignment) lines.push(`${indent}  ${varName}.TextYAlignment = Enum.TextYAlignment.${elem.text_y_alignment}`);
        if (elem.text_scaled) lines.push(`${indent}  ${varName}.TextScaled = true`);
      }
      // Image properties
      if (elem.image) {
        lines.push(`${indent}  ${varName}.Image = ${jq(elem.image)}`);
        const ic = colorExpr(elem.image_color);
        if (ic) lines.push(`${indent}  ${varName}.ImageColor3 = ${ic}`);
        if (elem.image_transparency != null) lines.push(`${indent}  ${varName}.ImageTransparency = ${Number(elem.image_transparency)}`);
        if (elem.scale_type) lines.push(`${indent}  ${varName}.ScaleType = Enum.ScaleType.${elem.scale_type}`);
      }
      // ScrollingFrame properties
      if (cls === "ScrollingFrame") {
        if (elem.canvas_size) lines.push(`${indent}  ${varName}.CanvasSize = ${udim2(elem.canvas_size)}`);
        if (elem.scroll_bar_thickness != null) lines.push(`${indent}  ${varName}.ScrollBarThickness = ${Number(elem.scroll_bar_thickness)}`);
        if (elem.automatic_canvas_size) lines.push(`${indent}  ${varName}.AutomaticCanvasSize = Enum.AutomaticSize.${elem.automatic_canvas_size}`);
      }
      // Decorations: UICorner
      if (elem.corner) {
        const cVar = `c${guiElemCount++}`;
        lines.push(`${indent}  local ${cVar} = Instance.new("UICorner")`);
        const r = elem.corner.radius || "0,8";
        lines.push(`${indent}  ${cVar}.CornerRadius = ${udim(r, "UDim.new(0, 8)")}`);
        lines.push(`${indent}  ${cVar}.Parent = ${varName}`);
      }
      // Decorations: UIStroke
      if (elem.stroke) {
        const sVar = `s${guiElemCount++}`;
        lines.push(`${indent}  local ${sVar} = Instance.new("UIStroke")`);
        const sc = colorExpr(elem.stroke.color || "255,255,255");
        if (sc) lines.push(`${indent}  ${sVar}.Color = ${sc}`);
        if (elem.stroke.thickness != null) lines.push(`${indent}  ${sVar}.Thickness = ${Number(elem.stroke.thickness)}`);
        if (elem.stroke.transparency != null) lines.push(`${indent}  ${sVar}.Transparency = ${Number(elem.stroke.transparency)}`);
        if (elem.stroke.mode) lines.push(`${indent}  ${sVar}.ApplyStrokeMode = Enum.ApplyStrokeMode.${elem.stroke.mode}`);
        lines.push(`${indent}  ${sVar}.Parent = ${varName}`);
      }
      // Decorations: UIGradient
      if (elem.gradient && elem.gradient.colors && elem.gradient.colors.length) {
        const gVar = `g${guiElemCount++}`;
        lines.push(`${indent}  local ${gVar} = Instance.new("UIGradient")`);
        const colorSeq = elem.gradient.colors.map((c) => {
          const ce = colorExpr(c);
          return ce ? `ColorSequenceKeypoint.new(0, ${ce})` : null;
        }).filter(Boolean);
        if (colorSeq.length === 1) {
          // Single color: just use it as both ends
          lines.push(`${indent}  ${gVar}.Color = ColorSequence.new(${colorExpr(elem.gradient.colors[0])})`);
        } else if (colorSeq.length >= 2) {
          // Multi-color: distribute evenly
          const keypoints = elem.gradient.colors.map((c, i) => {
            const t = elem.gradient.offsets && elem.gradient.offsets[i] != null
              ? elem.gradient.offsets[i]
              : i / (elem.gradient.colors.length - 1);
            return `ColorSequenceKeypoint.new(${t}, ${colorExpr(c)})`;
          });
          lines.push(`${indent}  ${gVar}.Color = ColorSequence.new({${keypoints.join(", ")}})`);
        }
        if (elem.gradient.rotation != null) lines.push(`${indent}  ${gVar}.Rotation = ${Number(elem.gradient.rotation)}`);
        lines.push(`${indent}  ${gVar}.Parent = ${varName}`);
      }
      // Decorations: UIPadding
      if (elem.padding) {
        const pVar = `p${guiElemCount++}`;
        lines.push(`${indent}  local ${pVar} = Instance.new("UIPadding")`);
        if (elem.padding.top) lines.push(`${indent}  ${pVar}.PaddingTop = ${udim(elem.padding.top)}`);
        if (elem.padding.bottom) lines.push(`${indent}  ${pVar}.PaddingBottom = ${udim(elem.padding.bottom)}`);
        if (elem.padding.left) lines.push(`${indent}  ${pVar}.PaddingLeft = ${udim(elem.padding.left)}`);
        if (elem.padding.right) lines.push(`${indent}  ${pVar}.PaddingRight = ${udim(elem.padding.right)}`);
        lines.push(`${indent}  ${pVar}.Parent = ${varName}`);
      }
      // Layout: UIListLayout or UIGridLayout
      if (elem.layout) {
        const lType = (elem.layout.type || "List").toString();
        if (lType === "Grid") {
          const lVar = `l${guiElemCount++}`;
          lines.push(`${indent}  local ${lVar} = Instance.new("UIGridLayout")`);
          if (elem.layout.cell_size) lines.push(`${indent}  ${lVar}.CellSize = ${udim2(elem.layout.cell_size, "UDim2.new(0, 100, 0, 100)")}`);
          if (elem.layout.cell_padding) lines.push(`${indent}  ${lVar}.CellPadding = ${udim2(elem.layout.cell_padding, "UDim2.new(0, 8, 0, 8)")}`);
          if (elem.layout.sort_order) lines.push(`${indent}  ${lVar}.SortOrder = Enum.SortOrder.${elem.layout.sort_order}`);
          if (elem.layout.horizontal_alignment) lines.push(`${indent}  ${lVar}.HorizontalAlignment = Enum.HorizontalAlignment.${elem.layout.horizontal_alignment}`);
          if (elem.layout.vertical_alignment) lines.push(`${indent}  ${lVar}.VerticalAlignment = Enum.VerticalAlignment.${elem.layout.vertical_alignment}`);
          lines.push(`${indent}  ${lVar}.Parent = ${varName}`);
        } else {
          const lVar = `l${guiElemCount++}`;
          lines.push(`${indent}  local ${lVar} = Instance.new("UIListLayout")`);
          lines.push(`${indent}  ${lVar}.FillDirection = Enum.FillDirection.${(elem.layout.direction || "Vertical")}`);
          if (elem.layout.padding) lines.push(`${indent}  ${lVar}.Padding = ${udim(elem.layout.padding, "UDim.new(0, 4)")}`);
          if (elem.layout.horizontal_alignment) lines.push(`${indent}  ${lVar}.HorizontalAlignment = Enum.HorizontalAlignment.${elem.layout.horizontal_alignment}`);
          if (elem.layout.vertical_alignment) lines.push(`${indent}  ${lVar}.VerticalAlignment = Enum.VerticalAlignment.${elem.layout.vertical_alignment}`);
          if (elem.layout.sort_order) lines.push(`${indent}  ${lVar}.SortOrder = Enum.SortOrder.${elem.layout.sort_order}`);
          lines.push(`${indent}  ${lVar}.Parent = ${varName}`);
      ...
      // Constraint: UIAspectRatioConstraint
      if (elem.aspect_ratio) {
        const aVar = `a${guiElemCount++}`;
        lines.push(`${indent}  local ${aVar} = Instance.new("UIAspectRatioConstraint")`);
        if (elem.aspect_ratio.ratio != null) lines.push(`${indent}  ${aVar}.AspectRatio = ${Number(elem.aspect_ratio.ratio)}`);
        if (elem.aspect_ratio.dominance) lines.push(`${indent}  ${aVar}.DominantAxis = Enum.DominantAxis.${elem.aspect_ratio.dominance}`);
        lines.push(`${indent}  ${aVar}.Parent = ${varName}`);
      }
      // Recurse into children
      if (Array.isArray(elem.children)) {
        for (const child of elem.children) {
          const childLines = guiElementLuau(child, varName, depth + 1);
    ...
    // ── GUI prefab templates ──
    // Each template is a function(overrides) -> {name, elements} that the
    // create_gui_template handler expands and feeds into the same guiElementLuau
    // pipeline as create_gui. Overrides let the AI customise colors/titles.
    const GUI_TEMPLATES = {
      shop: (ov) => {
        const accent = ov.accent_color || "0,200,255";
        const bg = ov.bg_color || "18,18,24";
        const card = ov.card_color || "30,30,42";
        const text = ov.text_color || "255,255,255";
        const title = ov.title || "SHOP";
        return {
          name: ov.name || "ShopGui",
          elements: [
            { class: "Frame", name: "Background", size: "1,0,1,0", position: "0,0,0,0", background_color: bg, background_transparency: 0.1,
              children: [
                { class: "Frame", name: "Header", size: "1,0,0,70", position: "0,0,0,0", background_color: accent, background_transparency: 0.15,
                  corner: { radius: "0,0" },
                  children: [
                    { class: "TextLabel", name: "Title", size: "0.5,0,1,0", position: "0.02,0,0,0", background_transparency: 1, text: title, text_color: text, text_size: 28, font: "GothamBold", text_x_alignment: "Left" },
                    { class: "TextLabel", name: "CoinsLabel", size: "0.3,0,1,0", position: "0.68,0,0,0", background_transparency: 1, text: "Coins: 0", text_color: "255,215,0", text_size: 22, font: "GothamSemibold", text_x_alignment: "Right" },
                    { class: "TextButton", name: "CloseBtn", size: "0,40,0,40", position: "0.95,10,0.5,-20", anchor_point: "0.5,0.5", background_color: "239,68,68", text: "X", text_color: text, text_size: 20, font: "GothamBold",
                      corner: { radius: "0,8" } }
                  ]
                },
                { class: "ScrollingFrame", name: "ItemsGrid", size: "0.9,0,0.85,0", position: "0.05,0,0.12,0", background_transparency: 1, scroll_bar_thickness: 6, canvas_size: "0,0,0,0", automatic_canvas_size: "Y",
                  layout: { type: "Grid", cell_size: "0,160,0,200", cell_padding: "0,12,0,12" },
                  children: [
                    { class: "Frame", name: "ItemCard1", size: "0,160,0,200", background_color: card,
                      corner: { radius: "0,12" }, stroke: { color: accent, thickness: 2 },
                      children: [
                        { class: "ImageLabel", name: "Icon", size: "0.8,0,0.45,0", position: "0.1,0,0.05,0", background_transparency: 1, image: "rbxassetid://0" },
                        { class: "TextLabel", name: "ItemName", size: "0.9,0,0,24", position: "0.05,0,0.55,0", background_transparency: 1, text: "Item Name", text_color: text, text_size: 16, font: "GothamSemibold" },
                        { class: "TextLabel", name: "Price", size: "0.9,0,0,20", position: "0.05,0,0.72,0", background_transparency: 1, text: "50 Coins", text_color: "255,215,0", text_size: 14, font: "Gotham" },
                        { class: "TextButton", name: "BuyBtn", size: "0.8,0,0,32", position: "0.1,0,0.82,0", background_color: accent, text: "BUY", text_color: text, text_size: 16, font: "GothamBold",
                          corner: { radius: "0,8" }, gradient: { colors: [accent, "0,150,255"], rotation: 90 } }
                      ]
                    },
                    { class: "Frame", name: "ItemCard2", size: "0,160,0,200", background_color: card,
                      corner: { radius: "0,12" }, stroke: { color: accent, thickness: 2 },
                      children: [
                        { class: "ImageLabel", name: "Icon", size: "0.8,0,0.45,0", position: "0.1,0,0.05,0", background_transparency: 1, image: "rbxassetid://0" },
                        { class: "TextLabel", name: "ItemName", size: "0.9,0,0,24", position: "0.05,0,0.55,0", background_transparency: 1, text: "Item Name", text_color: text, text_size: 16, font: "GothamSemibold" },
                        { class: "TextLabel", name: "Price", size: "0.9,0,0,20", position: "0.05,0,0.72,0", background_transparency: 1, text: "100 Coins", text_color: "255,215,0", text_size: 14, font: "Gotham" },
                        { class: "TextButton", name: "BuyBtn", size: "0.8,0,0,32", position: "0.1,0,0.82,0", background_color: accent, text: "BUY", text_color: text, text_size: 16, font: "GothamBold",
                          corner: { radius: "0,8" }, gradient: { colors: [accent, "0,150,255"], rotation: 90 } }
                      ]
                    }
      ...
      hud: (ov) => {
        const accent = ov.accent_color || "0,200,255";
        const bg = ov.bg_color || "0,0,0";
        const text = ov.text_color || "255,255,255";
        const healthColor = ov.health_color || "239,68,68";
        return {
          name: ov.name || "HudGui",
          elements: [
            { class: "Frame", name: "TopBar", size: "1,0,0,50", position: "0,0,0,0", background_color: bg, background_transparency: 0.3,
              children: [
                { class: "TextLabel", name: "CoinsDisplay", size: "0,120,0,40", position: "0.02,0,0.5,-20", anchor_point: "0,0.5", background_transparency: 1, text: "0 Coins", text_color: "255,215,0", text_size: 20, font: "GothamBold", text_x_alignment: "Left" },
                { class: "TextLabel", name: "LevelDisplay", size: "0,100,0,40", position: "0.5,0,0.5,-20", anchor_point: "0.5,0.5", background_transparency: 1, text: "Lv. 1", text_color: text, text_size: 20, font: "GothamBold" },
                { class: "TextLabel", name: "ScoreDisplay", size: "0,120,0,40", position: "0.98,0,0.5,-20", anchor_point: "1,0.5", background_transparency: 1, text: "Score: 0", text_color: text, text_size: 20, font: "GothamBold", text_x_alignment: "Right" }
              ]
            },
            { class: "Frame", name: "HealthBarBg", size: "0,250,0,24", position: "0.02,0,0.92,0", anchor_point: "0,1", background_color: bg, background_transparency: 0.2,
              corner: { radius: "0,12" }, stroke: { color: accent, thickness: 2 },
              children: [
                { class: "Frame", name: "HealthFill", size: "1,-6,1,-6", position: "0,3,0,3", background_color: healthColor,
                  corner: { radius: "0,10" } },
                { class: "TextLabel", name: "HealthText", size: "1,0,1,0", background_transparency: 1, text: "100 / 100", text_color: text, text_size: 16, font: "GothamBold" }
              ]
            },
            { class: "Frame", name: "MinimapPlaceholder", size: "0,150,0,150", position: "0.98,0,0.08,0", anchor_point: "1,0", background_color: bg, background_transparency: 0.3,
              corner: { radius: "0,12" }, stroke: { color: accent, thickness: 2 },
              children: [
                { class: "TextLabel", name: "MapLabel", size: "1,0,1,0", background_transparency: 1, text: "MAP", text_color: text, text_size: 14, font: "Gotham" }
              ]
            }
          ]
        };
      ...
      settings: (ov) => {
        const accent = ov.accent_color || "0,200,255";
        const bg = ov.bg_color || "18,18,28";
        const panel = ov.panel_color || "30,30,42";
        const text = ov.text_color || "255,255,255";
        const title = ov.title || "SETTINGS";
        return {
          name: ov.name || "SettingsGui",
          elements: [
            { class: "Frame", name: "DimBackground", size: "1,0,1,0", background_color: "0,0,0", background_transparency: 0.5 },
            { class: "Frame", name: "Panel", size: "0,500,0,400", position: "0.5,0,0.5,0", anchor_point: "0.5,0.5", background_color: panel,
              corner: { radius: "0,16" }, stroke: { color: accent, thickness: 2 },
              children: [
                { class: "Frame", name: "Header", size: "1,0,0,60", position: "0,0,0,0", background_color: accent, background_transparency: 0.2,
                  corner: { radius: "0,16" },
                  children: [
                    { class: "TextLabel", name: "Title", size: "0.7,0,1,0", position: "0.05,0,0,0", background_transparency: 1, text: title, text_color: text, text_size: 24, font: "GothamBold", text_x_alignment: "Left" },
                    { class: "TextButton", name: "CloseBtn", size: "0,40,0,40", position: "0.92,10,0.5,-20", anchor_point: "0.5,0.5", background_color: "239,68,68", text: "X", text_color: text, text_size: 18, font: "GothamBold",
                      corner: { radius: "0,8" } }
                  ]
                },
                { class: "Frame", name: "Content", size: "1,-40,0,300", position: "0.04,0,0.18,0", background_transparency: 1,
                  layout: { type: "List", direction: "Vertical", padding: "0,15" },
                  children: [
                    { class: "Frame", name: "MusicRow", size: "1,0,0,50", background_color: bg, background_transparency: 0.5,
                      corner: { radius: "0,10" },
                      children: [
                        { class: "TextLabel", name: "MusicLabel", size: "0.4,0,1,0", position: "0.05,0,0,0", background_transparency: 1, text: "Music Volume", text_color: text, text_size: 18, font: "GothamSemibold", text_x_alignment: "Left" },
                        { class: "Frame", name: "SliderBg", size: "0.4,0,0,8", position: "0.5,0,0.5,-4", anchor_point: "0,0.5", background_color: "60,60,70",
                          corner: { radius: "0,4" },
                          children: [
                            { class: "Frame", name: "SliderFill", size: "0.7,0,1,0", background_color: accent,
                              corner: { radius: "0,4" } }
                          ]
                        }
                      ]
                    },
                    { class: "Frame", name: "SFXRow", size: "1,0,0,50", background_color: bg, background_transparency: 0.5,
                      corner: { radius: "0,10" },
                      children: [
                        { class: "TextLabel", name: "SFXLabel", size: "0.4,0,1,0", position: "0.05,0,0,0", background_transparency: 1, text: "SFX Volume", text_color: text, text_size: 18, font: "GothamSemibold", text_x_alignment: "Left" },
                        { class: "Frame", name: "SliderBg2", size: "0.4,0,0,8", position: "0.5,0,0.5,-4", anchor_point: "0,0.5", background_color: "60,60,70",
                          corner: { radius: "0,4" },
                          children: [
                            { class: "Frame", name: "SliderFill2", size: "0.8,0,1,0", background_color: accent,
                              corner: { radius: "0,4" } }
                          ]
                        }
                      ]
                    },
                    { class: "Frame", name: "ToggleRow", size: "1,0,0,50", background_color: bg, background_transparency: 0.5,
                      corner: { radius: "0,10" },
                      children: [
                        { class: "TextLabel", name: "ShadowsLabel", size: "0.5,0,1,0", position: "0.05,0,0,0", background_transparency: 1, text: "Shadows", text_color: text, text_size: 18, font: "GothamSemibold", text_x_alignment: "Left" },
                        { class: "TextButton", name: "ToggleBtn", size: "0,60,0,30", position: "0.88,0,0.5,-15", anchor_point: "1,0.5", background_color: accent, text: "ON", text_color: text, text_size: 16, font: "GothamBold",
                          corner: { radius: "0,6" } }
                      ]
                    }
                  ]
      ...
      dialog: (ov) => {
        ...
        const bg = ov.bg_color || "18,18,28";
        const text = ov.text_color || "255,255,255";
        const npcName = ov.title || "NPC Name";
        const dialogText = ov.dialog_text || "Hello, traveler! What brings you here?";
        return {
          name: ov.name || "DialogGui",
          elements: [
            { class: "Frame", name: "DialogBox", size: "0.7,0,0,160", position: "0.15,0,0.8,0", anchor_point: "0,0", background_color: bg, background_transparency: 0.1,
              corner: { radius: "0,16" }, stroke: { color: accent, thickness: 3 },
              children: [
                { class: "Frame", name: "NameBar", size: "0.3,0,0,40", position: "0.04,0,0,-20", background_color: accent,
                  corner: { radius: "0,10" },
                  children: [
                    { class: "TextLabel", name: "NPCName", size: "1,0,1,0", background_transparency: 1, text: npcName, text_color: text, text_size: 20, font: "GothamBold" }
                  ]
                },
                { class: "TextLabel", name: "DialogText", size: "0.9,0,0,70", position: "0.05,0,0.25,0", background_transparency: 1, text: dialogText, text_color: text, text_size: 18, font: "Gotham", text_wraps: true, text_x_alignment: "Left", text_y_alignment: "Top" },
                { class: "TextButton", name: "NextBtn", size: "0,120,0,40", position: "0.75,0,0.72,0", background_color: accent, text: "Next >", text_color: text, text_size: 18, font: "GothamBold",
                  corner: { radius: "0,10" }, gradient: { colors: [accent, "0,150,255"], rotation: 90 } },
                { class: "TextButton", name: "CloseBtn", size: "0,100,0,40", position: "0.05,0,0.72,0", background_color: "60,60,70", text: "Close", text_color: text, text_size: 18, font: "GothamBold",
                  corner: { radius: "0,10" } }
              ]
            }
          ]
      ...
      loading: (ov) => {
        const accent = ov.accent_color || "0,200,255";
        const bg = ov.bg_color || "10,10,18";
        const text = ov.text_color || "255,255,255";
        const statusText = ov.status_text || "Loading...";
        const title = ov.title || "GAME NAME";
        return {
          name: ov.name || "LoadingGui",
          elements: [
            { class: "Frame", name: "Background", size: "1,0,1,0", background_color: bg, background_transparency: 0.05,
              children: [
                { class: "TextLabel", name: "GameTitle", size: "0.6,0,0,60", position: "0.2,0,0.3,0", background_transparency: 1, text: title, text_color: text, text_size: 48, font: "GothamBlack",
                  gradient: { colors: [accent, "150,0,255"], rotation: 0 } },
                { class: "Frame", name: "SpinnerBg", size: "0,60,0,60", position: "0.5,0,0.5,-80", anchor_point: "0.5,0.5", background_transparency: 1,
                  children: [
                    { class: "ImageLabel", name: "Spinner", size: "1,0,1,0", background_transparency: 1, image: "rbxassetid://0", image_color: accent }
                  ]
                },
                { class: "Frame", name: "ProgressBg", size: "0.5,0,0,12", position: "0.25,0,0.65,0", background_color: "40,40,50",
                  corner: { radius: "0,6" },
                  children: [
                    { class: "Frame", name: "ProgressFill", size: "0.3,0,1,0", background_color: accent,
                      corner: { radius: "0,6" }, gradient: { colors: [accent, "0,150,255"], rotation: 0 } }
                  ]
                },
                { class: "TextLabel", name: "StatusText", size: "0.5,0,0,30", position: "0.25,0,0.72,0", background_transparency: 1, text: statusText, text_color: text, text_size: 18, font: "GothamSemibold", text_x_alignment: "Left" },
                { class: "TextLabel", name: "PercentText", size: "0.1,0,0,30", position: "0.88,0,0.72,0", background_transparency: 1, text: "30%", text_color: accent, text_size: 18, font: "GothamBold", text_x_alignment: "Right" }
              ]
            }
          ]
        };
    ...
    if (name === "create_gui_template") {
      const tplName = (args.template || "").toString().toLowerCase();
      if (!GUI_TEMPLATES[tplName]) return `ERROR: create_gui_template: unknown template '${tplName}'. Available: shop, hud, settings, dialog, loading.`;
      const ov = (args.overrides && typeof args.overrides === "object") ? args.overrides : {};
      const tpl = GUI_TEMPLATES[tplName](ov);
      const guiName = (args.name || tpl.name).toString();
      const parentPath = (args.parent || "StarterGui").toString();
      const elements = tpl.elements || [];
      if (!elements.length) return `ERROR: create_gui_template: template '${tplName}' produced no elements.`;
      guiElemCount = 0;
      let code = `-- CoreLua create_gui_template: ${guiName} (template: ${tplName})\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local parent = safePath(${jq(parentPath)}) or game:GetService("StarterGui")\n`;
      code += `local gui = Instance.new("ScreenGui")\n`;
      code += `gui.Name = ${jq(guiName)}\n`;
      code += `gui.ResetOnSpawn = ${args.reset_on_spawn === true ? "true" : "false"}\n`;
      code += `gui.IgnoreGuiInset = ${args.ignore_gui_inset === false ? "false" : "true"}\n`;
      code += `gui.Parent = parent\n`;
      for (const elem of elements) {
        try {
          code += guiElementLuau(elem, "gui", 0) + "\n";
        } catch (e) {
          return `ERROR: create_gui_template element '${elem.name || elem.class || "?"}': ${e.message}`;
        }
      }
      code += `return {ok = true, gui = gui:GetFullName(), elements = ${guiElemCount}}\n`;
      const out = await runLua(code);
      return `Output of 'create_gui_template':\nCreated ScreenGui "${guiName}" (template: ${tplName}) under ${parentPath} with ${guiElemCount} element(s).\n${out}`;
    }

    if (name === "create_gui") {
      const guiName = (args.name || "CoreLuaGUI").toString();
      const parentPath = (args.parent || "StarterGui").toString();
      const elements = Array.isArray(args.elements) ? args.elements : [];
      if (!elements.length) return `ERROR: create_gui needs a non-empty 'elements' array.`;
      guiElemCount = 0;
      let code = `-- CoreLua create_gui: ${guiName}\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local parent = safePath(${jq(parentPath)}) or game:GetService("StarterGui")\n`;
      code += `local gui = Instance.new("ScreenGui")\n`;
      code += `gui.Name = ${jq(guiName)}\n`;
      code += `gui.ResetOnSpawn = ${args.reset_on_spawn === true ? "true" : "false"}\n`;
      if (args.display_order != null) code += `gui.DisplayOrder = ${Number(args.display_order)}\n`;
      code += `gui.IgnoreGuiInset = ${args.ignore_gui_inset === false ? "false" : "true"}\n`;
      code += `gui.Parent = parent\n`;
      // Build each top-level element
      for (const elem of elements) {
        try {
          code += guiElementLuau(elem, "gui", 0) + "\n";
        } catch (e) {
          return `ERROR: create_gui element '${elem.name || elem.class || "?"}': ${e.message}`;
        }
      }
      code += `return {ok = true, gui = gui:GetFullName(), elements = ${guiElemCount}}\n`;
      const out = await runLua(code);
      return `Output of 'create_gui':\nCreated ScreenGui "${guiName}" under ${parentPath} with ${guiElemCount} element(s).\n${out}`;
    }


    // Helper: infer the Roblox type of a property value string and emit Luau.
    // Handles Vector3, CFrame, Color3, UDim2, UDim, NumberRange, BrickColor,
    // Enum values, booleans, numbers, and plain strings.
    function propLuau(key, val) {
      const v = val.trim();
      // Boolean
      if (v === "true" || v === "false") return `ok, err = setProp(inst, ${jq(key)}, ${v}); if not ok then return {ok = false, error = err} end\n`;
      // Number
      if (/^-?\d+(\.\d+)?$/.test(v)) return `ok, err = setProp(inst, ${jq(key)}, ${v}); if not ok then return {ok = false, error = err} end\n`;
      // Vector3: x,y,z
      if (/^-?\d+(\.\d+)?,-?\d+(\.\d+)?,-?\d+(\.\d+)?$/.test(v)) {
        const p = v.split(",").map((x) => parseFloat(x.trim()));
        return `ok, err = setProp(inst, ${jq(key)}, Vector3.new(${p[0]}, ${p[1]}, ${p[2]})); if not ok then return {ok = false, error = err} end\n`;
      }
      // Color3: r,g,b (0-255)
      if (/^\d{1,3},\d{1,3},\d{1,3}$/.test(v)) {
        const p = v.split(",").map((x) => parseInt(x.trim()));
        if (p.every((x) => x >= 0 && x <= 255)) {
          return `ok, err = setProp(inst, ${jq(key)}, Color3.fromRGB(${p[0]}, ${p[1]}, ${p[2]})); if not ok then return {ok = false, error = err} end\n`;
        }
      }
      // UDim2: sx,ox,sy,oy
      if (/^-?\d+(\.\d+)?,-?\d+,-?\d+(\.\d+)?,-?\d+$/.test(v)) {
        const p = v.split(",").map((x) => parseFloat(x.trim()));
        return `ok, err = setProp(inst, ${jq(key)}, UDim2.new(${p[0]}, ${p[1]}, ${p[2]}, ${p[3]})); if not ok then return {ok = false, error = err} end\n`;
      }
      // Enum: try Enum.X.Y or bare enum name
      if (v.startsWith("Enum.")) {
        return `ok, err = setProp(inst, ${jq(key)}, ${v}); if not ok then return {ok = false, error = err} end\n`;
      }
      // Default: string
      return `ok, err = setProp(inst, ${jq(key)}, ${jq(v)}); if not ok then return {ok = false, error = err} end\n`;
    }
    ...
    if (name === "delete_instance") {
      const targets = Array.isArray(args.targets) ? args.targets.map(String) : [];
      if (!targets.length) return `ERROR: delete_instance needs a non-empty 'targets' array.`;
      if (args.confirm !== true) return `ERROR: delete_instance requires 'confirm: true' to proceed. This is a safety guard against accidental deletion. Re-issue with confirm=true to actually delete.`;
      // UI confirmation toast: warn the user BEFORE the destruction runs.
      const tgtList = targets.length <= 3 ? targets.join(", ") : targets.slice(0, 3).join(", ") + ` +${targets.length - 3} more`;
      ui.toast({ msg: `Deleting ${targets.length} instance(s): ${tgtList}`, type: "warning", timeout: 6000 });
      let code = `-- CoreLua delete_instance\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local deleted, missing = 0, 0\n`;
      code += `for _, p in ipairs(${jq(targets)}) do local inst = safePath(p) if inst then inst:Destroy() deleted = deleted + 1 else missing = missing + 1 end end\n`;
      code += `return {ok = true, deleted = deleted, missing = missing}\n`;
      const out = await runLua(code);
      return `Output of 'delete_instance':\nDeleted ${targets.length} target(s).\n${out}`;
    }

    if (name === "duplicate_instance") {
      const src = (args.source || "").toString();
      if (!src) return `ERROR: duplicate_instance needs 'source' (dot-path of the instance to clone).`;
      const newName = args.new_name ? String(args.new_name) : null;
      const parentPath = args.parent ? String(args.parent) : null;
      const offset = args.position_offset ? String(args.position_offset) : null;
      let code = `-- CoreLua duplicate_instance\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local src = safePath(${jq(src)})\n`;
      code += `if not src then return {ok = false, error = "Source not found: ${src}"} end\n`;
      code += `local clone = src:Clone()\n`;
      if (newName) code += `clone.Name = ${jq(newName)}\n`;
      if (parentPath) {
        code += `local np = safePath(${jq(parentPath)}) or game:GetService("Workspace")\n`;
        code += `clone.Parent = np\n`;
      } else {
        code += `clone.Parent = src.Parent\n`;
      }
      if (offset) {
        const parts = offset.split(",").map((x) => parseFloat(x.trim()));
        code += `if clone:IsA("BasePart") then clone.CFrame = clone.CFrame + Vector3.new(${parts[0]||0}, ${parts[1]||0}, ${parts[2]||0})\n`;
        code += `elseif clone:IsA("Model") and clone.PrimaryPart then clone:PivotTo(clone.WorldPivot + Vector3.new(${parts[0]||0}, ${parts[1]||0}, ${parts[2]||0})) end\n`;
      }
      code += `return {ok = true, clone = clone:GetFullName()}\n`;
      const out = await runLua(code);
      return `Output of 'duplicate_instance':\nCloned "${src}"${newName ? " as \"" + newName + "\"" : ""}.\n${out}`;
    }

    if (name === "set_property") {
      const target = (args.target || "").toString();
      if (!target) return `ERROR: set_property needs 'target' (dot-path).`;
      const props = args.properties || {};
      const propEntries = Object.entries(props);
      if (!propEntries.length) return `ERROR: set_property needs a non-empty 'properties' object.`;
      // Build Luau that sets each property with type inference.
      let code = `-- CoreLua set_property\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local inst = safePath(${jq(target)})\n`;
      code += `if not inst then return {ok = false, error = "Instance not found: ${target}"} end\n`;
      code += `local setProp = function(obj, key, val)\n`;
      code += `  local parts = {}\n`;
      code += `  for s in string.gmatch(key, "[^.]+") do table.insert(parts, s) end\n`;
      code += `  for i = 1, #parts - 1 do obj = obj[parts[i]]; if not obj then return false, "missing sub-property " .. parts[i] end end\n`;
      code += `  obj[parts[#parts]] = val\n`;
      code += `  return true\n`;
      code += `end\n`;
      code += `local ok, err\n`;
      for (const [key, val] of propEntries) {
        code += `-- Property: ${key} = ${val}\n`;
        code += propLuau(key, String(val));
      }
      code += `return {ok = true, instance = inst:GetFullName()}\n`;
      const out = await runLua(code);
      return `Output of 'set_property':\nSet ${propEntries.length} propert${propEntries.length > 1 ? "ies" : "y"} on "${target}".\n${out}`;
    }

    if (name === "move_instance") {
      const target = (args.target || "").toString();
      if (!target) return `ERROR: move_instance needs 'target' (dot-path).`;
      const newParent = args.new_parent ? String(args.new_parent) : null;
      const position = args.position ? String(args.position) : null;
      const rotation = args.rotation ? String(args.rotation) : null;
      const offset = args.offset ? String(args.offset) : null;
      let code = `-- CoreLua move_instance\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local inst = safePath(${jq(target)})\n`;
      code += `if not inst then return {ok = false, error = "Instance not found: ${target}"} end\n`;
      if (newParent) {
        code += `local np = safePath(${jq(newParent)})\n`;
        code += `if np then inst.Parent = np else return {ok = false, error = "Parent not found: ${newParent}"} end\n`;
      }
      // Build the new CFrame
      if (position || rotation || offset) {
        code += `if inst:IsA("BasePart") then\n`;
        if (offset) {
          const o = offset.split(",").map((x) => parseFloat(x.trim()));
          code += `  inst.CFrame = inst.CFrame + Vector3.new(${o[0]||0}, ${o[1]||0}, ${o[2]||0})\n`;
        }
        if (position) {
          const p = position.split(",").map((x) => parseFloat(x.trim()));
          code += `  inst.Position = Vector3.new(${p[0]||0}, ${p[1]||0}, ${p[2]||0})\n`;
        }
        if (rotation) {
          const r = rotation.split(",").map((x) => parseFloat(x.trim()));
          code += `  inst.CFrame = inst.CFrame * CFrame.Angles(math.rad(${r[0]||0}), math.rad(${r[1]||0}), math.rad(${r[2]||0}))\n`;
        }
        code += `elseif inst:IsA("Model") then\n`;
        code += `  local cf = inst.WorldPivot\n`;
        if (offset) {
          const o = offset.split(",").map((x) => parseFloat(x.trim()));
          code += `  cf = cf + Vector3.new(${o[0]||0}, ${o[1]||0}, ${o[2]||0})\n`;
        }
        if (position) {
          const p = position.split(",").map((x) => parseFloat(x.trim()));
          code += `  cf = CFrame.new(${p[0]||0}, ${p[1]||0}, ${p[2]||0})\n`;
        }
        if (rotation) {
          const r = rotation.split(",").map((x) => parseFloat(x.trim()));
          code += `  cf = cf * CFrame.Angles(math.rad(${r[0]||0}), math.rad(${r[1]||0}), math.rad(${r[2]||0}))\n`;
        }
        code += `  inst:PivotTo(cf)\n`;
        code += `end\n`;
      }
      code += `return {ok = true, path = inst:GetFullName()}\n`;
      const out = await runLua(code);
      return `Output of 'move_instance':\nMoved "${target}".\n${out}`;
    }

    if (name === "insert_from_toolbox") {
      const assetId = parseInt(args.asset_id);
      if (!assetId || isNaN(assetId)) return `ERROR: insert_from_toolbox needs a valid numeric 'asset_id'.`;
      const parentPath = (args.parent || "Workspace").toString();
      const nameStr = args.name ? String(args.name) : null;
      let code = `-- CoreLua insert_from_toolbox\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local parent = safePath(${jq(parentPath)}) or game:GetService("Workspace")\n`;
      code += `local ts = game:GetService("InsertService")\n`;
      code += `local models, err = pcall(function() return ts:LoadAsset(${assetId}) end)\n`;
      code += `if not models then return {ok = false, error = tostring(err)} end\n`;
      code += `local inserted = nil\n`;
      code += `if typeof(models) == "Instance" then\n`;
      code += `  if models:IsA("Folder") then\n`;
      code += `    for _, child in ipairs(models:GetChildren()) do child.Parent = parent; inserted = child end\n`;
      code += `    models:Destroy()\n`;
      code += `  else\n`;
      code += `    models.Parent = parent; inserted = models\n`;
      code += `  end\n`;
      code += `end\n`;
      if (nameStr) code += `if inserted then inserted.Name = ${jq(nameStr)} end\n`;
      code += `return {ok = true, path = inserted and inserted:GetFullName() or "unknown"}\n`;
      const out = await runLua(code);
      return `Output of 'insert_from_toolbox':\nInserted asset ${assetId} under ${parentPath}.\n${out}`;
    }

    if (name === "create_animation") {
      const animName = (args.name || "Animation").toString();
      const parentPath = (args.parent || "ReplicatedStorage.Animations").toString();
      const loop = args.loop === true;
      const priority = (args.priority || "Action").toString();
      const keyframes = Array.isArray(args.keyframes) ? args.keyframes : [];
      if (!keyframes.length) return `ERROR: create_animation needs a non-empty 'keyframes' array.`;
      let code = `-- CoreLua create_animation\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local parent = safePath(${jq(parentPath)})\n`;
      code += `if not parent then\n`;
      code += `  -- Auto-create the Animations folder\n`;
      code += `  local rs = game:GetService("ReplicatedStorage")\n`;
      code += `  parent = rs:FindFirstChild("Animations") or Instance.new("Folder")\n`;
      code += `  if not parent.Parent then parent.Name = "Animations"; parent.Parent = rs end\n`;
      code += `end\n`;
      code += `local anim = Instance.new("Animation")\n`;
      code += `anim.Name = ${jq(animName)}\n`;
      code += `anim.Parent = parent\n`;
      code += `-- Build the KeyframeSequence\n`;
      code += `local kfs = Instance.new("KeyframeSequence")\n`;
      code += `kfs.Name = ${jq(animName)}\n`;
      code += `kfs.Loop = ${loop}\n`;
      code += `kfs.Priority = Enum.AnimationPriority.${priority}\n`;
      for (const kf of keyframes) {
        const t = parseFloat(kf.time) || 0;
        code += `do\n  local kf = Instance.new("Keyframe")\n  kf.Time = ${t}\n`;
        if (Array.isArray(kf.poses)) {
          for (const pose of kf.poses) {
            const part = String(pose.part || "HumanoidRootPart");
            code += `  local pose = Instance.new("Pose")\n  pose.Name = ${jq(part)}\n`;
            if (pose.position) {
              const p = String(pose.position).split(",").map((x) => parseFloat(x.trim()));
              code += `  pose.CFrame = CFrame.new(${p[0]||0}, ${p[1]||0}, ${p[2]||0})\n`;
            }
            if (pose.rotation) {
              const r = String(pose.rotation).split(",").map((x) => parseFloat(x.trim()));
              code += `  pose.CFrame = pose.CFrame * CFrame.Angles(math.rad(${r[0]||0}), math.rad(${r[1]||0}), math.rad(${r[2]||0}))\n`;
            }
            code += `  kf:AddPose(pose)\n`;
          }
        }
        code += `  kfs:AddKeyframe(kf)\nend\n`;
      }
      code += `-- Save the KeyframeSequence as a child of the Animation for reference\n`;
      code += `kfs.Parent = anim\n`;
      code += `return {ok = true, animation = anim:GetFullName(), keyframes = ${keyframes.length}}\n`;
      const out = await runLua(code);
      return `Output of 'create_animation':\nCreated animation "${animName}" with ${keyframes.length} keyframe(s) under ${parentPath}.\n${out}`;
    }

    if (name === "create_remote_event") {
      const parentPath = (args.parent || "ReplicatedStorage.Remotes").toString();
      const remotes = Array.isArray(args.remotes) ? args.remotes : [];
      if (!remotes.length) return `ERROR: create_remote_event needs a non-empty 'remotes' array.`;
      let code = `-- CoreLua create_remote_event\n`;
      code += `local safePath = function(p) local o = game; for part in string.gmatch(p, "[^.]+") do o = o and o:FindFirstChild(part); if not o then return nil end end; return o end\n`;
      code += `local parent = safePath(${jq(parentPath)})\n`;
      code += `if not parent then\n`;
      code += `  -- Auto-create the folder structure\n`;
      code += `  local rs = game:GetService("ReplicatedStorage")\n`;
      code += `  local parts = {}\n`;
      code += `  for s in string.gmatch(${jq(parentPath)}, "[^.]+") do table.insert(parts, s) end\n`;
      code += `  parent = rs\n`;
      code += `  for i = 2, #parts do\n`;
      code += `    local child = parent:FindFirstChild(parts[i])\n`;
      code += `    if not child then child = Instance.new("Folder"); child.Name = parts[i]; child.Parent = parent end\n`;
      code += `    parent = child\n`;
      code += `  end\n`;
      code += `end\n`;
      code += `local created = {}\n`;
      for (const r of remotes) {
        const rName = String(r.name || "Remote");
        const rType = String(r.type || "RemoteEvent");
        code += `do\n  local r = Instance.new(${jq(rType)})\n  r.Name = ${jq(rName)}\n  r.Parent = parent\n  table.insert(created, r:GetFullName())\nend\n`;
      }
      code += `return {ok = true, created = created}\n`;
      const out = await runLua(code);
      return `Output of 'create_remote_event':\nCreated ${remotes.length} remote(s) under ${parentPath}.\n${out}`;
    }

    // ── code_gen_luau ──
    // The model already writes Luau well; this tool is a STRUCTURED prompt that
    // makes it produce a full script and create it in one go. We do NOT generate
    // the code ourselves (the AI is better at it) - we instruct it. So we return
    // a tailored instruction that the loop injects as the "result", nudging the
    // model to call multi_edit with the generated script.
    if (name === "code_gen_luau") {
      const path = (args.path || "").toString();
      const klass = (args.class || "ModuleScript").toString();
      const spec = (args.spec || "").toString();
      const style = (args.style || "typed").toString();
      if (!path || !spec) return `ERROR: code_gen_luau needs 'path' and 'spec'.`;
      return `Output of 'code_gen_luau':\nInstruction acknowledged. Now write a complete ${klass} at ${path} that satisfies this spec, then CREATE it with multi_edit (className "${klass}", first edit old_string "" with the full source). Requirements:\n- Idiomatic Luau for Roblox; ${style === "typed" ? "use type annotations for public functions and key locals" : style === "oop" ? "use OOP classes with a __index metatable and .new constructor" : style === "functional" ? "keep it functional, no classes" : "keep it minimal and direct"}.\n- Roblox best practices: task.* over wait(), no Instance.new without an immediate Parent, services via GetService, no global state, early returns, guard nils.\n- Include sensible error handling (pcall around risky calls, asserts for preconditions).\n- Spec: ${spec}\nDo NOT just describe the code - actually generate the FULL source and write it with multi_edit in your next reply.`;
    }

    // ── code_refactor ──
    if (name === "code_refactor") {
      const path = (args.path || "").toString();
      const goals = (args.goals || "").toString();
      if (!path) return `ERROR: code_refactor needs 'path'.`;
      return `Output of 'code_refactor':\nInstruction acknowledged. Refactor the script at ${path}:\n1. script_read ${path} first to get the exact current source.\n2. Improve: readability (early returns, clear names, small functions), performance (avoid repeated GetService/FindFirstChild in loops, cache tables), and Roblox best practices (task.* not wait(), no Instance.new without parent, type annotations where helpful, no leaking globals, no yielding on the main thread).${goals ? `\n3. Extra goals: ${goals}` : ""}\n3. Apply the result with multi_edit (replace the whole script body or targeted edits - your call, but keep behavior identical).\n4. Reply with a short summary of what you changed and why. Do not just describe - actually read and rewrite it now.`;
    }
    ...
    if (name === "code_review") {
      ...
      if (!path) return `ERROR: code_review needs 'path'.`;
      return `Output of 'code_review':\nInstruction acknowledged. Review the script at ${path}:\n1. script_read ${path} first.\n2. Return a structured report (no edits) covering: BUGS (logic errors, nil derefs, race conditions), ANTI-PATTERNS (wait() vs task.wait(), Instance.new without parent, global state), PERFORMANCE (repeated lookups, unnecessary loops, table churn), ROBLOX GOTCHAS (yielding on main thread, unanchored parts, RemoteEvent server/client security, exploitability of client-fired remotes), STYLE (naming, types, dead code).${focus !== "all" ? ` Focus especially on: ${focus}.` : ""}\n3. For each issue: severity (high/med/low), the line/area, and a concrete fix suggestion. Do NOT modify the script.`;
    }

    // ── code_add_tests ──
    if (name === "code_add_tests") {
      const modulePath = (args.module_path || "").toString();
      const cases = (args.cases || "").toString();
      if (!modulePath) return `ERROR: code_add_tests needs 'module_path'.`;
      // Derive a sibling test script path.
      const parts = modulePath.split(".");
      const last = parts.pop();
      const testPath = parts.join(".") + "." + last + ".tests";
      return `Output of 'code_add_tests':\nInstruction acknowledged. Generate a Luau test harness for the ModuleScript at ${modulePath}:\n1. script_read ${modulePath} first to understand its API.\n2. Write a self-contained ${"`"}Script${"`"} (no external test framework) at ${testPath} that requires the module and runs assertions using a tiny built-in runner: local pass, fail = 0, 0; local function check(name, cond) if cond then pass = pass + 1 else fail = fail + 1 warn("FAIL: "..name) end end. Wrap each test in pcall so one failure doesn't stop the rest. Print a summary at the end ("X passed, Y failed").${cases ? `\n3. Cover these cases: ${cases}` : `\n3. Cover the module's main public functions with sensible inputs + edge cases (nil, empty, boundary values).`}\n4. CREATE it with multi_edit (className "Script", first edit old_string "" with the full source). Do not just describe - generate the full harness and write it now.`;
    }

    if (A.toolNames.size && !A.toolNames.has(name)) {
      return CL.FEEDBACK.unknownTool(name, [...A.toolNames]);
    }
    ...
    if (!r) return CL.FEEDBACK.bridgeOffline;
    ...
    if (r.ok && /Unable to find an active Studio instance|previously active Studio has disconnected/i.test(r.text || "")) {
      ...
      return CL.FEEDBACK.studioOffline;
    }
    ...
    if (r.ok) {
      ...
      if (r.images && r.images.length) {
        // Show the capture in a left-hand CoreLua popup (from the in-memory
    ...
    if (r.kind === "disconnected") return CL.FEEDBACK.bridgeOffline;
    if (r.kind === "timeout") {
    ...
    if (name === "execute_luau") {
      const err = r.error || "";
      // Detect the classic JS-in-Luau mistake: the model wrote JavaScript array
      // methods (which do NOT exist on Luau tables) and got "attempt to call
      // missing method 'map'/'filter'/... of table". Tell it exactly what to do.
      const jsMethodMatch = err.match(/missing method '(\w+)' of table/);
      const jsMethods = ["map", "filter", "split", "join", "find", "findIndex", "forEach", "reduce", "some", "every", "includes", "indexOf", "slice", "flat", "flatMap", "concat", "reverse", "sort", "keys", "values", "entries", "at", "fill"];
      let hint;
      if (err.includes("Failed to parse command code")) {
        hint = "Your code block was empty or the marker was wrong. Use exactly ###LUA### (three hashes) - never ###LUA---. The code must be between ###LUA### and ###END_LUA###.";
      } else if (jsMethodMatch && jsMethods.includes(jsMethodMatch[1])) {
        const m = jsMethodMatch[1];
        const luauAlt = ({
          map: "a generic for/ipairs loop that builds a new table",
          filter: "a for/ipairs loop with an if that copies matching entries into a new table",
          split: "string.split(s, sep) (Roblox) or string.gmatch",
          join: "table.concat(t, sep)",
          find: "a for/ipairs loop that returns the first match, or table.find(t, value)",
          findIndex: "a for/ipairs loop that returns the index of the first match",
          forEach: "a plain for/ipairs loop",
          reduce: "a for/ipairs loop with an accumulator local",
          some: "a for/ipairs loop that returns true on the first passing entry",
          every: "a for/ipairs loop that returns false on the first failing entry",
          includes: "table.find(t, value) ~= nil",
          indexOf: "table.find(t, value)",
          slice: "a for loop from i to j copying into a new table",
          flat: "nested for/ipairs loops",
          flatMap: "a for/ipairs loop that extends a result table",
          concat: "a for/ipairs loop appending each element (or table.move)",
          reverse: "a backward for loop into a new table",
          sort: "table.sort(t, comparator)",
          keys: "a for loop with ipairs/pairs collecting indices/keys",
          values: "a for loop with ipairs/pairs collecting values",
          entries: "a for loop with pairs collecting {k, v} pairs",
          at: "t[(n > 0 and n or #t + n + 1)]",
          fill: "a for loop setting t[i] = value",
        })[m] || "a for/ipairs loop";
        hint = `CRITICAL Luau mistake: you called the JavaScript method '.${m}()' on a Luau table. Luau tables do NOT have '${m}'. This is JavaScript, not Luau. Replace it with ${luauAlt}. Never use .map/.filter/.split/.join/.find/.forEach/.reduce/.includes on a Luau table - Luau has NO JavaScript-style array methods. Use for/ipairs loops and the 'table' library (table.concat, table.find, table.insert, table.remove, table.sort, table.move) and string.* functions (string.split on Roblox, string.gmatch, string.match, string.find) instead. Rewrite the code in idiomatic Luau and retry.`;
      } else if (err.includes("attempt to") || err.includes("nil value")) {
        hint = "Lua runtime error. Check that the API you are calling exists (use game:GetService() to access services). Make sure you use 'return' to output values, not 'print()'. Remember Luau has NO JavaScript array methods (.map/.filter/.split/.join/.find/.forEach) - use for/ipairs loops and the 'table' library instead.";
      } else {
        hint = "Check your Lua syntax, make sure you use 'return' to output values (not 'print()'), and that all APIs you call exist in the current Roblox Studio context. Remember Luau has NO JavaScript array methods (.map/.filter/.split/.join/.find/.forEach) - use for/ipairs loops and the 'table' library instead.";
      }
  ...
  async function agentLoop(base) {
    if (A.running) return;
    A.running = true;
    A.loopStart = Date.now(); // for the "session finished" desktop notification
    ...
    try {
      while (!A.stop) {
        ...
        if (res.kind === "truncated") {
          ...
          if (truncCount < MAX_TRUNC) {
            ...
            ui.toast({ msg: "Reply was cut off, resuming…", type: "info" });
            base = await submitAndGetBase(CL.FEEDBACK.truncated);
            continue;
          }
        ...
        if (res.kind === "parse_error") {
          ...
          const failName = CLParse.toolNameFromText(res.raw || "") || "command";
          if (res.item) {
            const detail = res.reason === "unclosed" ? "cut off"
              : res.reason === "luaOpener" ? "missing ###LUA###"
              : res.reason === "envelope" ? "bad format"
              : "bad JSON";
            decorate.toolBox(res.item, failName, "err", detail, true, "", CL.toolCategory(failName));
          }
          ...
          base = await submitAndGetBase(CL.FEEDBACK.parseError(res.reason, failName));
          continue;
        }
        ...
        if (res.kind === "tool") {
          ...
          if (calls.length > 1) {
            base = await submitAndGetBase(CL.FEEDBACK.multiTool(calls.map((c) => c.tool || "?")));
            continue;
          }
          const call = calls[0];
          ...
          const category = learnedImg ? "screen" : CL.toolCategory(call.tool);
          ...
          // Toast: notify the user when a tool finishes (especially long ones)
          if (!feedback.startsWith("ERROR")) {
            ui.toast({ msg: `${bareToolName(call.tool)} executed`, type: "success", timeout: 2500 });
          } else {
            ui.toast({ msg: `${bareToolName(call.tool)} failed`, type: "error" });
          }
          if (A.stop) {
          ...
          if (!isErr && call.tool !== "list_commands" && A.toolList.length) {
            ...
            if (A.toolCallsSinceReminder >= REMIND_TOOLS_EVERY) {
              ...
              toSend += CL.toolsReminder(roblox) + "\n" + CL.memoryNudge();
      ...
      diag("loop.end");
      // Desktop notification: only fire for sessions that ran long enough to be
      // worth alerting about (>= 30s) AND only when the AI tab is NOT currently
      // visible - if the user is watching the chat, a toast is enough. Routed
      // through the background service worker because content scripts can't call
      // chrome.notifications.create directly.
      try {
        const dur = A.loopStart ? Date.now() - A.loopStart : 0;
        if (dur >= 30000 && document.hidden) {
          const secs = Math.round(dur / 1000);
          bg({ type: "notify", title: "CoreLua \u2014 session finished",
               message: `The agent finished after ${secs}s. Come back to the tab to see the result.` });
        }
      } catch {}
      A.loopStart = 0;
    }
  ...
  function stopLoop() {
    ...
    if (A.toolRunning && A.toolItem) {
      ...
      decorate.toolBox(A.toolItem, A.toolName, "err", "stopped", true, "", CL.toolCategory(A.toolName));
    }
    ui.markStopping();    // instant feedback: button → "⏳ Stopping…", disabled
    P.stopGeneration();
    ui.toast({ msg: "Stopping…", type: "warning", timeout: 2000 });
  }
  ...
  async function startSession() {
    ...
    if (!P.chatIsEmpty() && !A.started) {
      ui.toast({ msg: "Open a new, empty conversation to start a session.", type: "warning" });
      return;
    }
    A.userStopped = false;
    ...
    try {
      ...
      const prompt = CL.buildSystemPrompt({ siteName: P.displayName, customPrompt: ui.getCustomPrompt() });
      ...
      ui.setStarted(true);
      // Claude-specific proactive warning: Claude has a strict context/token
      // limit on the web UI. Show a persistent overlay banner so the user
      // knows to keep conversations short (user-requested feature).
      if (P.id === "claude") {
        ui.banner("token-limit", "Attention \u2014 Claude a une limite de token !",
          "Claude limite la taille du contexte. Si la conversation devient trop longue, " +
          "il faudra commencer une nouvelle conversation pour continuer \u00e0 utiliser l'agent CoreLua.");
      }
      ui.toast({ msg: `Agent ready - ${P.displayName}`, type: "info", timeout: 3000 });
    } catch (e) {
  ...
  const SPIN = '<span class="cl-spin"></span>';
  ...
  function resetDecoration(item) {
    const chip = item.querySelector(".cl-chip");
    if (chip) chip.remove();
    item.classList.remove("cl-hidden");
    item.querySelectorAll(".cl-tool-hide").forEach((e) => e.classList.remove("cl-tool-hide"));
    item.querySelectorAll(".cl-cmd-mask").forEach((e) => e.classList.remove("cl-cmd-mask"));
  ...
  const decorate = {
    ...
    chip(item, opts) {
      ...
      let chip = item.querySelector(".cl-chip");
      const hasBody = !!body;
      ...
      // <span class="cl-spin"> and restarts its CSS animation each time, so the
      ...
      if (chip.dataset.csig !== sig) {
        ...
        chip.className = `cl-chip cat-${category} ${cls || ""}`;
        chip.innerHTML =
          `<div class="cl-chip-head">` +
            `<span class="cl-chip-ic">${iconFor(category, phase)}</span>` +
            `<span class="cl-chip-tx"></span>` +
            `<span class="cl-chip-dt"></span>` +
            (hasBody ? `<span class="cl-chip-cv">${SVG('<polyline points="6 9 12 15 18 9"/>')}</span>` : "") +
          `</div>` +
          (hasBody ? `<div class="cl-chip-body"><pre></pre></div>` : "");
        chip.querySelector(".cl-chip-tx").textContent = label;
        if (detail) chip.querySelector(".cl-chip-dt").textContent = detail;
        if (hasBody) {
          chip.querySelector(".cl-chip-body pre").textContent = body;
          const head = chip.querySelector(".cl-chip-head");
      ...
      if (whole) {
        ...
        item.classList.add("cl-hidden");
      } else {
        item.classList.remove("cl-hidden");
        // findToolBlockSpot ALSO applies the .cl-tool-hide classes (its real job);
    ...
    ensureOwnedChip(item) {
      ...
      if (!opts.whole) {
        ...
        rawVisible = [...item.querySelectorAll("pre, p, [class*='code'], .cm-line")].some(
          (e) => !e.closest(".cl-tool-hide") && !e.closest(".cl-chip") &&
                 ...
                 // the .cl-tool-hide class) live on a child wrapper instead.
                 ...
                 !e.querySelector(".cl-tool-hide") &&
                 CLParse.hasCommandShape(e.textContent || ""));
      }
    ...
    toolBox(item, name, phase, detail, owned, body, category) {
      ...
      this.chip(item, {
        ...
        category: category || CL.toolCategory(name), phase, cls,
      });
    ...
    classify(item, next) {
      ...
      const txt = P.classifyText(item, ".cl-chip"); // excludes thinking AND our chip
      ...
      // .cl-tool-hide classes are wiped while the dataset flags on the turn
      ...
      const chipGone = !item.querySelector(".cl-chip");

      // 1. System-prompt bootstrap turn → animated while starting, gear when done.
      if (txt.includes(CL.SYS_MARKER)) {
      ...
      if (P.isUserItem(item) && CLParse.isInjectedFeedback(txt)) {
        ...
        if (item.dataset.zsig !== sig || !item.classList.contains("cl-hidden") || chipGone) {
          this.chip(item, {
            label: m ? `${m[1]} · result` : "result",
            category: hasImg ? "screen" : m ? CL.toolCategory(m[1]) : "tool",
      ...
      if (P.isAssistantItem(item) && !CLParse.hasCommandShape(txt) &&
          next && P.isUserItem(next)) {
        const nt = P.classifyText(next, ".cl-chip");
      ...
      // ONLY in a real CoreLua session (started or bootstrapping). Without
      ...
      if (P.isAssistantItem(item) && CLParse.hasCommandShape(txt) &&
        ...
        if (item.dataset.zRegenLen) {
          ...
          if (!replaced && !expired) {
            const nm = CLParse.toolNameFromText(txt) || "command";
        ...
        if (stopped && item.dataset.zStopped !== "1") {
          item.dataset.zStopped = "1";
          diag("chip.rehalt", { name: CLParse.toolNameFromText(txt) });
        }
        ...
        if (!stopped && A.running && !A.toolRunning && A.toolSettle &&
            ...
            CLParse.toolNameFromText(txt) === A.toolName) {
        ...
        const resultAfter = next && P.isUserItem(next) &&
          CLParse.isInjectedFeedback(P.classifyText(next, ".cl-chip"));
        ...
        if (phase === "done" && next && P.isUserItem(next)) {
          ...
          if (CLParse.isInjectedFeedback(nt) && feedbackIsError(nt)) {
            phase = "err"; detail = "error";
            if (item.dataset.zphase !== "err") diag("chip.errSettle", { name: CLParse.toolNameFromText(txt) });
          }
        }
        ...
        const rawVisible = [...item.querySelectorAll("pre, p, [class*='code'], .cm-line")].some(
          (e) => !e.classList.contains("cl-tool-hide") && !e.closest(".cl-tool-hide") &&
                 !e.closest(".cl-chip") && !(P.thinkingSel && e.closest(P.thinkingSel)) &&
        ...
        const nm = CLParse.toolNameFromText(txt);
        const cat = A.imageTools.has(bareToolName(nm)) ? "screen" : undefined;
        const chipNow = item.querySelector(".cl-chip");
      ...
      if (haltedTurn && P.isAssistantItem(item) && item.dataset.zphase !== "err"
          && item.querySelector(".cl-chip")) {
        const tx = item.querySelector(".cl-chip-tx");
        const name = CLParse.toolNameFromText(txt) || (tx && tx.textContent) || "tool";
      ...
      if (item.dataset.zs || item.dataset.zphase || item.querySelector(".cl-chip")) {
    ...
    sweep() {
      ...
      for (const chip of document.querySelectorAll(".cl-chip.run")) {
        ...
        if (item && item.dataset.zphase !== "err") {
          const tx = chip.querySelector(".cl-chip-tx");
  ...
  // ════════════════════════════════════════════════════════════════════════
  const ui = (() => {
    let root, bar, dot, brandEl, stateEl, actionBtn, stopBtn, switchBtn, menuEl;
    let explorerTab, explorerPanel, explorerBody;
    ...
    function build() {
      ...
      // menu (other AI sites, custom prompt, support). No floating panel,
      // no overlay on the input - the composer stays fully usable for plain chat.
      root.innerHTML = `
        <div id="cl-bar">
          <span id="cl-dot" class="off" title=""></span>
          <span id="cl-brand">CoreLua <span class="cl-free">v${EXT_VERSION}</span></span>
          <span id="cl-state"></span>
          <button id="cl-action"></button>
          <button id="cl-stop" hidden>■ Stop</button>
          <button id="cl-switch" aria-label="Switch AI and options" title="Switch AI, custom prompt, support"><span id="cl-switch-name"></span><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button>
        </div>
        <div id="cl-menu" hidden></div>
        <button id="cl-explorer-tab" aria-label="Toggle Roblox Explorer" title="Roblox Studio Explorer">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
        </button>
        <div id="cl-explorer" hidden>
          <div id="cl-explorer-head">
            <span id="cl-explorer-title">Explorer</span>
            <button id="cl-explorer-refresh" aria-label="Refresh" title="Refresh">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>
            </button>
          </div>
          <div id="cl-explorer-search">
            <svg class="cl-exp-search-ic" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.5" y2="16.5"/></svg>
            <input id="cl-explorer-search-input" type="text" placeholder="Filter..." autocomplete="off" spellcheck="false" />
            <button id="cl-explorer-search-clear" aria-label="Clear" title="Clear" hidden>\u2715</button>
          </div>
          <ul id="cl-explorer-body"></ul>
        </div>
      `;
      document.documentElement.appendChild(root);
      bar = root.querySelector("#cl-bar");
      dot = root.querySelector("#cl-dot");
      brandEl = root.querySelector("#cl-brand");
      stateEl = root.querySelector("#cl-state");
      actionBtn = root.querySelector("#cl-action");
      stopBtn = root.querySelector("#cl-stop");
      switchBtn = root.querySelector("#cl-switch");
      const swName = root.querySelector("#cl-switch-name");
      if (swName) swName.textContent = P.displayName || P.id;
      menuEl = root.querySelector("#cl-menu");
      explorerTab = root.querySelector("#cl-explorer-tab");
      explorerPanel = root.querySelector("#cl-explorer");
      explorerBody = root.querySelector("#cl-explorer-body");
      bar.classList.add(`cl-prov-${P.id}`); // lets CSS tune per-site (e.g. font)
      ...
      document.documentElement.classList.add(`cl-site-${P.id}`);

      actionBtn.addEventListener("click", onActionClick);
      stopBtn.addEventListener("click", stopLoop);
      buildMenu();
      // The model button opens the shared options panel at the top.
      const toggleMenu = () => {
    ...
    function syncMenuPrompt() {
      const ta = root && root.querySelector("#cl-set-text");
    ...
    // ── The "more" menu (⋯) ─────────────────────────────────────────────────
    // One popover holding every secondary control: other AI sites and the
    // custom prompt. Opens above the bar.
    function buildMenu() {
      ...
      for (const s of AI_SITES) {
        ...
        const label = `<span class="cl-site-name"><span>${s.name}</span><span class="cl-site-host">${hostOf(s.url)}</span></span>`;
        sites += current
          ? `<div class="cl-site-opt cl-site-here">${label}<span class="cl-site-badge">active</span></div>`
          : `<button class="cl-site-opt" data-u="${s.url}">${label}<span class="cl-site-go">&rarr;</span></button>`;
      }
      const esc = (s) => String(s).replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]));
      const mergedServers = mergedMcpServers();
      // Roblox always heads the list - greyed out, no health dot (its own status
      // is already the main CoreLua dot elsewhere) and no remove button (it's
      // the primary server, protected bridge-side too).
      let mcpList =
        `<div class="cl-mcp-item cl-mcp-item-primary"><div class="cl-mcp-info"><span class="cl-mcp-name">Roblox Studio</span><span class="cl-mcp-url">primary - always connected</span></div></div>`;
      mergedServers.forEach((s, i) => {
        ...
        mcpList += `<div class="cl-mcp-item"><span class="cl-mcp-health cl-mcp-health-${healthClass}" title="${healthTitle}"></span><div class="cl-mcp-info"><span class="cl-mcp-name">${esc(s.name)}</span><span class="cl-mcp-url">${esc(s.command || s.id)}</span></div><button class="cl-mcp-remove" data-id="${esc(s.id)}" title="Remove">✕</button></div>`;
      });
      menuEl.innerHTML =
        `<div class="cl-menu-head"><span class="cl-menu-logo">CoreLua</span><span class="cl-menu-tag">v${EXT_VERSION}</span></div>
         <section class="cl-menu-sec">
           <div class="cl-sec-label"><span>Switch AI</span></div>
           ${sites}
         </section>
         <section class="cl-menu-sec">
           <div class="cl-sec-label"><span>Custom prompt</span></div>
           <div class="cl-menu-note">Added below the system prompt on every new session. The built-in prompt can't be edited.</div>
           <textarea id="cl-set-text" rows="4" placeholder="e.g. Always comment your Luau code. Prefer small modular scripts."></textarea>
           <div class="cl-set-row"><button id="cl-set-save">Save</button><span id="cl-set-status"></span></div>
         </section>
         <section class="cl-menu-sec">
           <div class="cl-sec-label"><span>MCP servers</span></div>
           <div class="cl-menu-note">Roblox Studio is always connected (primary). Add another MCP server (e.g. Blender, Sketchfab) as an addon - the bridge restarts briefly to load it. Experimental.</div>
           ${mcpList}
           <div class="cl-mcp-sep"></div>
           <input id="cl-mcp-name" class="cl-mcp-field" placeholder="Name, e.g. Blender" />
           <input id="cl-mcp-url" class="cl-mcp-field" placeholder="Start command, e.g. npx -y @some/mcp-server" />
           <div class="cl-set-row"><button id="cl-mcp-add">Add server</button><span id="cl-mcp-status"></span></div>
         </section>`;
      const open = (url) => { try { window.open(url, "_blank", "noopener"); } catch {} menuEl.hidden = true; };
      menuEl.querySelectorAll("button.cl-site-opt").forEach((b) =>
        b.addEventListener("click", () => open(b.dataset.u)));
      const ta = menuEl.querySelector("#cl-set-text");
      const saveBtn = menuEl.querySelector("#cl-set-save");
      const status = menuEl.querySelector("#cl-set-status");
      ...
      const mcpNameEl = menuEl.querySelector("#cl-mcp-name");
      const mcpUrlEl = menuEl.querySelector("#cl-mcp-url");
      const mcpStatus = menuEl.querySelector("#cl-mcp-status");
      const mcpAddBtn = menuEl.querySelector("#cl-mcp-add");
      ...
      function setMcpBusy(on, label) {
        ...
        menuEl.querySelectorAll(".cl-mcp-remove").forEach((b) => (b.disabled = on));
        mcpStatus.innerHTML = on
          ? `<span class="cl-mcp-spin-row"><span class="cl-mcp-spin"></span>${label || "Restarting bridge…"}</span>`
          : "";
      }

      menuEl.querySelectorAll(".cl-mcp-remove").forEach((b) =>
    ...
    // ── Roblox Explorer panel (right-side, collapsible) ─────────────────────
    // A small read-only mirror of Roblox Studio's Explorer window: the common
    // top-level services as root nodes, expanded lazily on click via the
    // `inspect_instance` MCP tool (same one the AI uses to read the place).
    // inspect_instance has no fixed JSON schema documented in this codebase
    // (it's Studio's own built-in tool) - parseExplorerChildren() is a
    // best-effort text parser that handles the common "- Name (ClassName)"
    // and JSON-array shapes, and falls back to showing the raw text.
    ...
    const EXP_ARROW_SVG = '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>';

    // className -> icon + color for Explorer tree nodes.
    // Small colored dot or emoji so the user can instantly distinguish Scripts,
    // Parts, GUIs, Lights, Tools, etc. at a glance.
    const EXP_ICONS = {
      // Scripts / code
      Script: { icon: "\u{1F4C4}", color: "#7ec8e3" },
      LocalScript: { icon: "\u{1F4C4}", color: "#7ec8e3" },
      ModuleScript: { icon: "\u{1F4C4}", color: "#5bb8a3" },
      // Geometry
      Part: { icon: "\u25A0", color: "#a0a4b0" },
      TrussPart: { icon: "\u25A0", color: "#b0b8c0" },
      WedgePart: { icon: "\u25B2", color: "#c0a0b0" },
      CornerWedgePart: { icon: "\u25B2", color: "#c0a8b0" },
      MeshPart: { icon: "\u2B1B", color: "#9098a8" },
      UnionOperation: { icon: "\u2B1B", color: "#8a9aba" },
      NegateOperation: { icon: "\u2B1B", color: "#6a7080" },
      // Models & grouping
      Model: { icon: "\u{1F4E6}", color: "#e8c87a" },
      Folder: { icon: "\u{1F4C2}", color: "#e8c87a" },
      Workspace: { icon: "\u{1F310}", color: "#818cf8" },
      // GUI
      ScreenGui: { icon: "\u{1F4BB}", color: "#c4a8e8" },
      Frame: { icon: "\u25AD", color: "#b8a8d8" },
      TextButton: { icon: "\u{1F518}", color: "#e8a8c4" },
      ImageButton: { icon: "\u{1F5BC}", color: "#d8a8e8" },
      TextBox: { icon: "\u270F", color: "#a8c8e8" },
      TextLabel: { icon: "\u{1F4DD}", color: "#a8b8d8" },
      ImageLabel: { icon: "\u{1F5BC}", color: "#b8a8d8" },
      ScrollingFrame: { icon: "\u29C9", color: "#c8a8e8" },
      UIListLayout: { icon: "\u2630", color: "#8898a8" },
      UIGridLayout: { icon: "\u25A6", color: "#8898a8" },
      UIPadding: { icon: "\u25A2", color: "#8898a8" },
      UICorner: { icon: "\u25CF", color: "#8898a8" },
      UIStroke: { icon: "\u25EF", color: "#8898a8" },
      UIGradient: { icon: "\u{1F308}", color: "#8898a8" },
      UIAspectRatioConstraint: { icon: "\u29C8", color: "#8898a8" },
      UISizeConstraint: { icon: "\u29C8", color: "#8898a8" },
      // Effects / lighting
      PointLight: { icon: "\u{1F4A1}", color: "#f0d878" },
      SpotLight: { icon: "\u{1F4A1}", color: "#f0d878" },
      SurfaceLight: { icon: "\u{1F4A1}", color: "#f0d878" },
      Lighting: { icon: "\u{1F506}", color: "#f0d878" },
      Atmosphere: { icon: "\u{1F30C}", color: "#a8a8d8" },
      Sky: { icon: "\u{1F319}", color: "#a8b8d8" },
      // Sound
      Sound: { icon: "\u{1F50A}", color: "#e8a8a8" },
      SoundService: { icon: "\u{1F509}", color: "#c89898" },
      SoundGroup: { icon: "\u{1F50A}", color: "#d8a8a8" },
      // Characters & players
      Player: { icon: "\u{1F3B4}", color: "#7ec8a3" },
      Players: { icon: "\u{1F465}", color: "#7ec8a3" },
      Humanoid: { icon: "\u{1F6B6}", color: "#7ec8a3" },
      HumanoidDescription: { icon: "\u{1F6B6}", color: "#88b898" },
      Accessory: { icon: "\u{1F451}", color: "#e8c878" },
      Shirt: { icon: "\u{1F455}", color: "#a8c8d8" },
      Pants: { icon: "\u{1FA73}", color: "#a8c8d8" },
      Tool: { icon: "\u{1F527}", color: "#e8b878" },
      HopperBin: { icon: "\u{1F527}", color: "#e8b878" },
      Backpack: { icon: "\u{1F392}", color: "#b8a878" },
      // Services
      ReplicatedStorage: { icon: "\u{1F4E4}", color: "#818cf8" },
      ReplicatedFirst: { icon: "\u{1F4E4}", color: "#818cf8" },
      ServerStorage: { icon: "\u{1F4E5}", color: "#a88bc8" },
      ServerScriptService: { icon: "\u{1F4C4}", color: "#a88bc8" },
      StarterGui: { icon: "\u{1F4BB}", color: "#c4a8e8" },
      StarterPack: { icon: "\u{1F392}", color: "#b8a878" },
      StarterPlayer: { icon: "\u{1F3B4}", color: "#7ec8a3" },
      Teams: { icon: "\u{1F465}", color: "#7ec8c8" },
      Chat: { icon: "\u{1F4AC}", color: "#7ec8d8" },
      // Constraints
      WeldConstraint: { icon: "\u{1F517}", color: "#8898a8" },
      Weld: { icon: "\u{1F517}", color: "#8898a8" },
      HingeConstraint: { icon: "\u{1F517}", color: "#8898a8" },
      // Misc
      Camera: { icon: "\u{1F4F7}", color: "#c8c8a8" },
      Terrain: { icon: "\u{1F30F}", color: "#a8c878" },
      Configuration: { icon: "\u2699", color: "#8898a8" },
      ValueBase: { icon: "\u{1F4AF}", color: "#8898a8" },
      StringValue: { icon: "\u{1F520}", color: "#a8c8e8" },
      IntValue: { icon: "#", color: "#a8c8e8" },
      NumberValue: { icon: "#", color: "#a8c8e8" },
      BooleanValue: { icon: "\u2713", color: "#a8c8e8" },
      ObjectValue: { icon: "\u{1F4CD}", color: "#a8c8e8" },
      CFrameValue: { icon: "\u{1F4CD}", color: "#a8c8e8" },
      Vector3Value: { icon: "\u{1F4CD}", color: "#a8c8e8" },
      Color3Value: { icon: "\u{1F3A8}", color: "#a8c8e8" },
      BindableEvent: { icon: "\u26A1", color: "#e8c878" },
      BindableFunction: { icon: "\u2699", color: "#e8c878" },
      RemoteEvent: { icon: "\u{1F4E8}", color: "#e8a878" },
      RemoteFunction: { icon: "\u{1F4E9}", color: "#e8a878" },
      Animation: { icon: "\u{1F3AC}", color: "#c8a8e8" },
      AnimationController: { icon: "\u{1F3AC}", color: "#c8a8e8" },
      Animator: { icon: "\u{1F3AC}", color: "#c8a8e8" },
      Motor6D: { icon: "\u{1F517}", color: "#8898a8" },
      BillboardGui: { icon: "\u{1F4CC}", color: "#c4a8e8" },
      SurfaceGui: { icon: "\u{1F4CC}", color: "#c4a8e8" },
    };
    // Resolve icon for a className, returning { icon, color } or a default dot.
    const expIconFor = (className) => {
      if (!className) return { icon: "\u25CF", color: "#6b7089" };
      // Try exact match first, then IsA-style family match by suffix.
      if (EXP_ICONS[className]) return EXP_ICONS[className];
      // ValueBase family
      if (/Value$/.test(className)) return EXP_ICONS.ValueBase;
      // Constraint family
      if (/Constraint$/.test(className)) return EXP_ICONS.WeldConstraint;
      // Light family
      if (/Light$/.test(className)) return EXP_ICONS.PointLight;
      // Default: small dot
      return { icon: "\u25CF", color: "#6b7089" };
    };


    function parseExplorerChildren(text) {
      const raw = String(text || "").trim();
      if (!raw) return [];
      // Property/meta labels that must never appear as a child node name.
      const ignoredNames = /^(Path|Full ?Path|Class ?Name|Instance|Properties|Attributes|Children|Parent|Name|Description|Source|Script|Locked|Archivable)$/i;
      const cleanName = (value) => String(value ?? "").replace(/^["'`]|["'`]$/g, "").trim();
      const cleanClass = (value) => String(value ?? "").replace(/^["'`]|["'`]$/g, "").trim();
      const itemFromValue = (value) => {
        if (typeof value === "string") {
          const s = value.trim();
          // "- Baseplate (Part)" / "* Camera [Camera]" / "> SpawnLocation"
          const m = s.match(/^(?:[-*\u2022>]\s*)?(.+?)\s*(?:\(([^()]+)\)|\[([^\]]+)\])\s*$/);
          return m
            ? { name: cleanName(m[1]), className: cleanClass(m[2] || m[3]) }
            : (s && !ignoredNames.test(s) ? { name: cleanName(s), className: "" } : null);
        }
        if (!value || typeof value !== "object" || Array.isArray(value)) return null;
        // Accept any of the common name field spellings.
        let name = value.name ?? value.Name ?? value.instanceName ?? value.InstanceName ?? value.instance ?? value.Instance;
        let className = value.className ?? value.ClassName ?? value.class ?? value.Class ?? value.type ?? value.Type ?? value.class_name;
        // Roblox Studio's real inspect_instance children often carry only
        // "path" and "className" (no "name" field). Derive the child name
        // from the last segment of the path: "Players.rasko494" -> "rasko494".
        if (name == null || !cleanName(name)) {
          const p = cleanName(value.path ?? value.Path ?? value.fullPath ?? value.FullPath ?? "");
          if (p) {
            const seg = p.split(/[.\\]/).filter(Boolean).pop();
            if (seg) name = seg;
          }
        }
        // If still no name but we have a path, the path itself is a usable label.
        if (name == null || !cleanName(name)) {
          const p = cleanName(value.path ?? value.Path ?? "");
          if (p) name = p;
        }
        return name != null && cleanName(name) && !ignoredNames.test(cleanName(name))
          ? { name: cleanName(name), className: cleanClass(className || "") }
          : null;
      };
      const fromArray = (items) => {
        if (!Array.isArray(items)) return [];
        return items.map(itemFromValue).filter((item) => item && item.name);
      };
      // Keys that directly hold the child collection. Roblox Studio's built-in
      // inspect_instance has used several of these across versions
      // (children, childInstances, childSummaries, childrenSummary, ...).
      const childKey = (obj) => {
        if (!obj || typeof obj !== "object" || Array.isArray(obj)) return null;
        return Object.keys(obj).find((key) =>
          /^(children|childInstances|childSummaries|childrenSummary|childSummary|instances|descendants|childList|kids|immediateChildren|immediateChildSummaries|immediateChildrenSummary|directChildren|directChildSummaries|immediateChildSummary)$/i.test(key));
      };
      const nestedKey = (obj) => {
        if (!obj || typeof obj !== "object" || Array.isArray(obj)) return null;
        return Object.keys(obj).find((key) => /^(result|data|instance|details|output|response|body|payload)$/i.test(key));
      };
      const fromJson = (value, depth = 0) => {
        if (depth > 6 || value == null) return [];
        if (Array.isArray(value)) return fromArray(value);
        if (typeof value !== "object") return [];
        const key = childKey(value);
        if (key) {
          const direct = Array.isArray(value[key]) ? fromArray(value[key]) : fromJson(value[key], depth + 1);
          if (direct.length) return direct;
          if (typeof value[key] === "string") return parseText(value[key]);
        }
        const wrapper = nestedKey(value);
        if (wrapper) {
          const wrapped = fromJson(value[wrapper], depth + 1);
          if (wrapped.length) return wrapped;
        }
        // Safety net: if this is an object that looks like a children wrapper
        // (has an array value somewhere but no recognized key was found),
        // scan all array-valued properties and return the first non-empty one.
        for (const k of Object.keys(value)) {
          if (Array.isArray(value[k]) && value[k].length) {
            const fromArr = fromArray(value[k]);
      ...
      const parseJsonCandidate = (candidate) => {
        try { return fromJson(JSON.parse(candidate)); } catch { return []; }
      };
      // MCP responses are sometimes prefixed with "Output of ..." or wrapped
      // in a markdown fence. Try the complete response first, then the first
      // balanced-looking JSON object/array inside it.
      const jsonCandidates = [raw];
      const firstJson = raw.search(/[\[{]/);
      if (firstJson > 0) jsonCandidates.push(raw.slice(firstJson).replace(/```(?:json)?\s*$/i, "").trim());
      ...
      function parseText(value) {
        const source = String(value || "").replace(/```(?:text|json)?/gi, "").replace(/```/g, "");
        const out = [];
        const seen = new Set();
        const add = (name, className = "") => {
          name = cleanName(name);
          className = cleanClass(className);
          if (!name || ignoredNames.test(name)) return;
          const key = name + "|" + className;
          if (!seen.has(key)) { seen.add(key); out.push({ name, className }); }
        };
        // Locate the "Children:" / "Children (N):" section if present.
        const childrenStart = source.search(/(?:^|\n)\s*children(?:\s*\([^)]*\))?\s*:/i);
        const hasChildrenSection = childrenStart >= 0;
        // Locate sections that describe the instance itself (Properties,
        // Attributes, etc.) so pair/line matching inside them is skipped.
        // Each section spans from its header line to the next header line at
        // the same indent level (or a Children section), whichever comes first.
        const sectionHeaders = ["properties", "attributes", "parent", "path", "instance", "class"];
        const headerRe = /(?:^|\n)([\t ]*)([A-Za-z][A-Za-z ]*?)\s*(?:\([^)]*\))?\s*:/g;
        const headers = [];
        let hm;
        while ((hm = headerRe.exec(source))) {
          const indent = hm[1].length;
          const label = hm[2].trim().toLowerCase();
          headers.push({ index: hm.index, indent, label, end: hm.index + hm[0].length });
        }
        // Build spans for instance-describing sections (Properties/Attributes/
        // Instance/Class/Parent/Path). A span ends at the next header with
        // indent <= this one (i.e. a sibling or outer section).
        const sectionRanges = [];
        for (let i = 0; i < headers.length; i++) {
          const h = headers[i];
          if (!sectionHeaders.some((s) => h.label === s || h.label.startsWith(s))) continue;
          if (h.label === "children") continue; // Children is NOT a skip section
          let end = source.length;
          for (let j = i + 1; j < headers.length; j++) {
            if (headers[j].indent <= h.indent) { end = headers[j].index; break; }
          }
          sectionRanges.push([h.index, end]);
        }
        const inSection = (idx) => {
          for (const [s, e] of sectionRanges) if (idx >= s && idx < e) return true;
          return false;
        };
        // Standard StudioMCP summary lines: "- Baseplate (Part)".
        const lineRe = /^[\t ]*(?:[-*\u2022>]\s*)?(.+?)\s*(?:\(([^()]+)\)|\[([^\]]+)\])\s*$/gm;
        let match;
        const childSectionEnd = hasChildrenSection
          ? (() => {
              const after = source.slice(childrenStart).split(/\r?\n/).slice(1);
              let end = childrenStart;
              for (const line of after) {
                end += line.length + 1;
                if (/^\s*(?:properties|attributes|descendants?|parent|path|instance|class)\s*:/i.test(line)) break;
              }
              return end;
            })()
          : source.length;
        while ((match = lineRe.exec(source))) {
          if (hasChildrenSection && (match.index < childrenStart || match.index >= childSectionEnd)) continue;
          // Reject lines that are part of a Properties/Attributes section.
          if (inSection(match.index)) continue;
          // Reject the instance's own header pair (column-0 "Instance: X (Y)").
          if (!hasChildrenSection) {
            const before = source.lastIndexOf("\n", match.index - 1) + 1;
            const lineStart = source.slice(before, match.index);
            if (/^(instance|class|path)\s*:/i.test(lineStart.trim())) continue;
          }
          add(match[1], match[2] || match[3] || "");
        }
        // Indented key/value pair blocks that describe children:
        //   Name: Camera / ClassName: Camera
        //   Instance Name: X / Class Name: Y
        // Skip any match that falls inside a Properties/Attributes section.
        const pairRe = /(?:^|\n)([\t ]+)(?:instance\s*name|name|instance)\s*:\s*(.+?)\s*\n[\t ]+(?:class\s*name|class(?:name)?|type)\s*:\s*(.+?)(?=\n|$)/gi;
        while ((match = pairRe.exec(source))) {
          if (inSection(match.index)) continue;
          add(match[2], match[3]);
        }
        // Repeated top-level (column-0) "Instance Name: X / Class Name: Y"
        // blocks: these represent a flat list of children when there is no
        // single-instance Properties dump. Only used when no Children section
        // was found and at least 2 such blocks exist (a single block is the
        // instance's own header, not a child).
        if (!hasChildrenSection) {
          const flatRe = /(?:^|\n)(?:instance\s*name|name)\s*:\s*(.+?)\s*\n(?:class\s*name|class(?:name)?|type)\s*:\s*(.+?)(?=\n|$)/gi;
          const flatMatches = [];
          while ((match = flatRe.exec(source))) flatMatches.push(match);
          if (flatMatches.length >= 2) for (const m of flatMatches) add(m[1], m[2]);
        }
        // Within a Children section, accept plain "  Baseplate" and
        // "  Baseplate: Part" entries that the lineRe pair matcher missed.
        if (hasChildrenSection) {
          const section = source.slice(childrenStart, childSectionEnd);
          for (const line of section.split(/\r?\n/).slice(1)) {
            const plain = line.match(/^\s*(?:[-*\u2022>]\s*)?([A-Za-z0-9_][^:\n]*?)\s*(?::\s*([A-Za-z0-9_].*))?\s*$/);
            if (plain) {
              const nm = plain[1].trim();
              const cls = (plain[2] || "").trim();
              if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(nm)) add(nm, cls);
            }
          }
        }
        return out;
    ...
    function explorerNodeHtml(path, name, className) {
      const ic = expIconFor(className);
      const icStyle = ic.color ? ` style="color:${ic.color}"` : "";
      return `<li class="cl-exp-node" data-path="${escHtml(path)}" data-name="${escHtml((name||"").toLowerCase())}" data-cls="${escHtml((className||"").toLowerCase())}">
        <div class="cl-exp-row">
          <button class="cl-exp-arrow">${EXP_ARROW_SVG}</button>
          <span class="cl-exp-ic"${icStyle}>${ic.icon}</span>
          <span class="cl-exp-name">${escHtml(name)}</span>
          <span class="cl-exp-cls">${escHtml(className || "")}</span>
        </div>
        <ul class="cl-exp-children" hidden></ul>
      </li>`;
    }

    async function explorerLoadChildren(li) {
      const path = li.dataset.path;
      const childUl = li.querySelector(":scope > .cl-exp-children");
      if (!childUl) return;
      // Guard against double-loading: if a request is already in flight for
      // this node, don't fire a second one (double-clicks were spawning
      // parallel inspect_instance calls that both wrote into the same <ul>).
      if (li.dataset.loading === "1") return;
      li.dataset.loading = "1";
      childUl.innerHTML = `<li class="cl-exp-msg"><span class="cl-spin"></span> Loading…</li>`;
      // Timeout wrapper: if inspect_instance doesn't respond within 20s the
      // bridge is likely busy or hung - show an error instead of an eternal
      // spinner. The user can click the arrow again to retry.
      let timedOut = false;
      const timeoutId = setTimeout(() => {
        timedOut = true;
        if (!li.isConnected) return;
        childUl.innerHTML = `<li class="cl-exp-msg cl-exp-err">Loading timed out (20s) — bridge may be busy. Click to retry.</li>`;
        li.dataset.loading = "";
        li.dataset.loaded = "";
      }, 20000);
      const r = await bg({ type: "call_tool", name: "inspect_instance", arguments: { path }, timeout: 15000 });
      clearTimeout(timeoutId);
      // The li may have been collapsed/removed while the request was in
      // flight - bail out quietly rather than writing into a stale node.
      if (timedOut || !li.isConnected) return;
      if (!r || !r.ok) {
        childUl.innerHTML = `<li class="cl-exp-msg cl-exp-err">${escHtml((r && (r.error || r.text)) || "no response")}</li>`;
        li.dataset.loading = "";
        return;
      }
      const kids = parseExplorerChildren(r.text);
      if (!kids.length) {
        // Distinguish "instance genuinely has no children" from "we could not
        // parse the response". Showing a truncated raw preview lets the user
        // see exactly what inspect_instance returned so they (or we) can add
        // a parser branch for a new format instead of seeing a dead-end
        // "unrecognized format" message.
        const raw = String(r.text || "").trim();
        let msgHtml;
        if (!raw) {
          msgHtml = `<li class="cl-exp-msg">No children (empty response)</li>`;
        } else if (/^\{\s*"instanceName"\s*:[^}]*\}\s*$/.test(raw) && !/children|child|descendant/i.test(raw)) {
          // Looks like the structuredContent stub leaked through with no text
          // payload (e.g. {"instanceName":"Workspace"}) - the instance may
          // simply have no children, or the bridge did not receive content[0].text.
          msgHtml = `<li class="cl-exp-msg">No children (stub response — check bridge logs)</li>`;
        } else {
          // Check if the response explicitly says there are zero children.
          // Roblox Studio's real format: "children":{"immediateChildren":[],"totalDescendants":0}
          let isGenuinelyEmpty = false;
          try {
            const obj = JSON.parse(raw);
            // Look for a children object with an empty array
            const childrenObj = obj.children ?? obj.childInstances ?? obj.immediateChildren;
            if (childrenObj && typeof childrenObj === "object" && !Array.isArray(childrenObj)) {
              const arr = childrenObj.immediateChildren ?? childrenObj.directChildren ?? childrenObj.childInstances ?? childrenObj.children;
              if (Array.isArray(arr) && arr.length === 0) isGenuinelyEmpty = true;
            }
            // Also check top-level immediateChildren/directChildren empty array
            if (!isGenuinelyEmpty) {
              for (const k of ["immediateChildren", "directChildren", "children", "childInstances"]) {
                if (Array.isArray(obj[k]) && obj[k].length === 0) { isGenuinelyEmpty = true; break; }
              }
            }
            // totalDescendants / totalChildren == 0 is a strong signal too
            if (!isGenuinelyEmpty) {
              const td = obj.totalDescendants ?? obj.totalChildren ?? obj.childrenCount;
              if (td === 0) isGenuinelyEmpty = true;
            }
          } catch { /* not JSON, fall through */ }
          if (isGenuinelyEmpty) {
            msgHtml = `<li class="cl-exp-msg">No children (leaf instance)</li>`;
          } else {
            const preview = raw.length > 240 ? raw.slice(0, 240) + "…" : raw;
            msgHtml = `<li class="cl-exp-msg cl-exp-err">No children parsed. Raw response:<br><code style="white-space:pre-wrap;word-break:break-all;font-size:11px;opacity:.85">${escHtml(preview)}</code></li>`;
          }
        }
        childUl.innerHTML = msgHtml;
        li.dataset.loaded = "1";
        li.dataset.loading = "";
        return;
      }
      childUl.innerHTML = kids.map((k) => explorerNodeHtml(`${path}.${k.name}`, k.name, k.className)).join("");
      li.dataset.loaded = "1";
      li.dataset.loading = "";
    }

    function explorerToggle(li) {
      const arrow = li.querySelector(":scope > .cl-exp-row > .cl-exp-arrow");
      const childUl = li.querySelector(":scope > .cl-exp-children");
      if (!childUl) return;
      const opening = childUl.hidden;
      childUl.hidden = !opening;
      if (arrow) arrow.classList.toggle("cl-exp-arrow-open", opening);
      if (opening && li.dataset.loaded !== "1") explorerLoadChildren(li);
    }


    function explorerRenderRoots() {
      explorerBody.innerHTML = EXPLORER_ROOTS.map((n) => explorerNodeHtml(n, n, "")).join("");
    }

    // Explorer search/filter: recursively show/hide tree nodes whose name OR
    // className matches the query (case-insensitive). A node is visible if it
    // OR any descendant matches; non-matching parents of matching children stay
    // visible (and get expanded) so the user sees the full path to the match.
    let explorerSearchQuery = "";
    function explorerApplySearch(li, q) {
      const name = li.dataset.name || "";
      const cls = li.dataset.cls || "";
      const selfMatch = !q || name.includes(q) || cls.includes(q);
      // Recurse into children first
      let childMatch = false;
      const childUl = li.querySelector(":scope > .cl-exp-children");
      if (childUl) {
        // Expand collapsed nodes so matches are visible
        if (q && childUl.hidden) {
          childUl.hidden = false;
          const arrow = li.querySelector(":scope > .cl-exp-row > .cl-exp-arrow");
          if (arrow) arrow.classList.add("cl-exp-arrow-open");
          if (li.dataset.loaded !== "1") explorerLoadChildren(li);
        }
        const children = childUl.querySelectorAll(":scope > .cl-exp-node");
        for (const child of children) {
          if (explorerApplySearch(child, q)) childMatch = true;
        }
        // Collapse non-matching empty subtrees when searching
        if (q && !childMatch && childUl.children.length > 0) {
          // Keep expanded if searching (user may want to see structure)
        }
      }
      const visible = selfMatch || childMatch;
      li.style.display = visible ? "" : "none";
      // Highlight matches
      const nameEl = li.querySelector(":scope > .cl-exp-row > .cl-exp-name");
      if (nameEl) {
        if (q && selfMatch && !childMatch) nameEl.classList.add("cl-exp-match");
        else nameEl.classList.remove("cl-exp-match");
      }
      return visible;
    }
    function explorerRunSearch(q) {
      explorerSearchQuery = q;
      const roots = explorerBody.querySelectorAll(":scope > .cl-exp-node");
      for (const root of roots) explorerApplySearch(root, q);
      // Show "no results" if everything is hidden
      let anyVisible = false;
      for (const r of roots) { if (r.style.display !== "none") { anyVisible = true; break; } }
      let noRes = explorerBody.querySelector("#cl-exp-no-results");
      if (!anyVisible && q) {
        if (!noRes) {
          noRes = document.createElement("li");
          noRes.id = "cl-exp-no-results";
          noRes.className = "cl-exp-msg";
          noRes.textContent = "No results";
          explorerBody.appendChild(noRes);
        }
        noRes.style.display = "";
      } else if (noRes) {
    ...
    function buildExplorer() {
      ...
      explorerBody.addEventListener("click", (e) => {
        const row = e.target.closest(".cl-exp-row");
        if (!row) return;
        const li = row.parentElement;
        if (li && li.classList.contains("cl-exp-node")) explorerToggle(li);
      });
      explorerTab.addEventListener("click", () => {
        explorerPanel.hidden = !explorerPanel.hidden;
        explorerTab.classList.toggle("cl-exp-tab-open", !explorerPanel.hidden);
      });
      root.querySelector("#cl-explorer-refresh").addEventListener("click", (e) => {
        e.stopPropagation();
        explorerRenderRoots();
        const si = root.querySelector("#cl-explorer-search-input");
        if (si) { si.value = ""; const cb = root.querySelector("#cl-explorer-search-clear"); if (cb) cb.hidden = true; explorerRunSearch(""); }
      });
      // Search input: filter as you type (debounced)
      const searchInput = root.querySelector("#cl-explorer-search-input");
      const searchClear = root.querySelector("#cl-explorer-search-clear");
      ...
      if (searchInput) {
        searchInput.addEventListener("input", (e) => {
          ...
          searchTimer = setTimeout(() => explorerRunSearch(v), 150);
        });
        searchInput.addEventListener("keydown", (e) => {
          if (e.key === "Escape") { e.target.value = ""; if (searchClear) searchClear.hidden = true; explorerRunSearch(""); e.target.blur(); }
        });
      }
      if (searchClear) {
        searchClear.addEventListener("click", () => {
          if (searchInput) { searchInput.value = ""; searchClear.hidden = true; explorerRunSearch(""); searchInput.focus(); }
        });
      }
    }
    ...
    function buildSetup() {
      ...
      setupCard.id = "cl-setup";
      setupCard.hidden = true;
      setupCard.innerHTML =
        `<div id="cl-setup-head"><span id="cl-setup-logo">CoreLua</span><span id="cl-setup-tag">Setup</span></div>` +
        `<div id="cl-setup-sub">The <b>Bridge</b> is what connects this chat to Roblox Studio. Two steps and you're running.</div>` +
        `<ol id="cl-setup-steps">` +
          `<li>Run <code>start.bat</code></li>` +
          `<li>Back here, click <b>Start Roblox agent</b></li>` +
        `</ol>` +
        `<button id="cl-setup-dismiss">Got it</button>`;
      document.documentElement.appendChild(setupCard);

      setupCard.querySelector("#cl-setup-dismiss").addEventListener("click", () => {
        setupSeen = true;
    ...
    function refreshSetup(bridgeConnected) {
      ...
      // conversation with no CoreLua session (the "No agent here" state),
    ...
    function renderBar() {
      ...
      if (A.starting) {
        toneClass = "starting";
        indicator = `<span class="cl-spin"></span>`;
        ...
        if (A.bridge && A.bridge.connected === false) {
          ...
          // session (warn=false → no cl-state-warn red text). The full nag
      ...
      // The AI selector and the support menu appear once the agent is actually
      // running - so the pre-start bar isn't cluttered with options that only
      // matter mid-session. (A.started is ambiguous with `warn` tone - which
      // occurs both started-with-bridge-down and standby-with-bridge-down - so
      // it's tracked explicitly in the signature.)
      ...
      lastBarSig = sig;
      // Set the tone WITHOUT clobbering other classes (e.g. cl-bar-inline, which
      ...
      stateEl.innerHTML = indicator + `<span class="cl-state-txt">${msg}</span>`;
      stateEl.classList.toggle("cl-state-warn", warn);
    ...
    function setStatus(s) {
      ...
      // CoreLua status tracks ONLY the primary Roblox MCP server. Every other
      ...
      if (wasConnected && !s.connected) {
        bridgeAlert(true);
        ui.toast({ msg: "Bridge disconnected - reconnecting automatically...", type: "warning", timeout: 5000 });
      }
      if (s.connected) {
        bridgeAlert(false);
        if (wasConnected === false) {
          // Bridge just came back after being offline - confirm reconnection.
          ui.toast({ msg: "Bridge reconnected - CoreLua is back online", type: "success", timeout: 4000 });
        }
      }
    ...
    function bridgeAlert(on) {
      ...
      b.className = "cl-banner limit";
      b.innerHTML = `<div class="cl-banner-t">⚠ Lost connection to CoreLua</div>
        <div class="cl-banner-m">The CoreLua bridge stopped on your PC. Restart it (run start.bat and keep Roblox Studio open): the agent will reconnect automatically as soon as it is detected again.</div>
        <div class="cl-banner-acts"><button class="cl-banner-x">Close</button></div>`;
      b.querySelector(".cl-banner-x").addEventListener("click", () => { b.remove(); if (bridgeBannerEl === b) bridgeBannerEl = null; });
      root.appendChild(b);
      bridgeBannerEl = b;
    }
    ...
    function showStop(v) {
      ...
      // Pulsing red glow while the AI is actively working (not winding down).
      if (allow && !A.stopping) {
        stopBtn.dataset.working = "1";
      } else {
        delete stopBtn.dataset.working;
      }
    ...
    function nudgeStart() {
      ...
      if (!nudged) {
        nudged = true;
        toast({ msg: "Tip: click “▶ Start Roblox agent” to let the AI control Roblox Studio.", type: "info", timeout: 5000 });
      }
      if (!actionBtn) return;
      actionBtn.classList.add("cl-flash");
      setTimeout(() => actionBtn.classList.remove("cl-flash"), 1200);
    }
    ...
    // `.cl-light` to <html>; overlay.css then flips to readable light colours.
    ...
    function applyTheme() {
      ...
      document.documentElement.classList.toggle("cl-light", light);
    }
    ...
    function placeBar() {
      ...
      // #cl-root from <html>, taking the bar with it - and nothing re-adds it, so
      ...
      if (mount) {
        ...
        if (!bar.classList.contains("cl-bar-inline")) {
          bar.classList.add("cl-bar-inline");
        ...
        bar.classList.toggle("cl-bar-inside", !!mount.inside);
      ...
      // node into (e.g. Kimi's Vue tree - inserting #cl-bar there makes Vue's
      // next diff reuse the bar node as a host and nest the editor inside it).
      // So we keep the bar in our own #cl-root, position it (position:fixed) to
      ...
      if (anchorEl && anchorEl.isConnected) {
        bar.classList.remove("cl-bar-inline", "cl-bar-inside");
        bar.classList.add("cl-bar-anchored");
        ...
        if (menuEl && !menuEl.hidden) {
          bar.classList.remove("cl-bar-inline"); // ensure fixed geometry for menu math
    ...
    function inputCover(on) {
      ...
      if (!on) {
        ...
        if (ed) ed.classList.remove("cl-typing");
        cancelAnimationFrame(coverRaf);
        return;
      }
      if (!ed) return;
      ed.classList.add("cl-typing"); // make the typed text itself invisible
      if (!cover) {
        cover = document.createElement("div");
        cover.id = "cl-input-cover";
      ...
      const place = () => {
        ...
        if (!e.classList.contains("cl-typing")) e.classList.add("cl-typing");
        ...
        // .cl-typing height cap, so the editor balloons to ~1500px while its
        ...
        // are unchanged.
        // Hard ceiling: even though .cl-typing caps the editor's visual height
    ...
    // Toast container: holds stacked toasts in the bottom-right corner.
    let toastContainer = null;
    function ensureToastContainer() {
      if (toastContainer && toastContainer.isConnected) return toastContainer;
      toastContainer = document.createElement("div");
      toastContainer.id = "cl-toast-stack";
      document.documentElement.appendChild(toastContainer);
      return toastContainer;
    }
    // Enhanced toast: supports types (success, error, info, warning) with
    // distinct icons and colors. Positioned bottom-right, stacked vertically.
    // Usage: toast(msg) or toast(msg, type) or toast({ msg, type, icon, timeout })
    function toast(msg, type) {
      let opts = msg;
      if (typeof msg === "string") {
        opts = { msg, type: type || "info" };
      } else if (!opts.type) {
        opts.type = "info";
      }
      const container = ensureToastContainer();
      const t = document.createElement("div");
      t.className = `cl-toast cl-toast-${opts.type}`;
      // Icon per type
      const icons = {
        success: "\u2713",
        error: "\u2715",
      ...
      const ic = opts.icon || icons[opts.type] || "\u2139";
      t.innerHTML = `<span class="cl-toast-ic">${ic}</span><span class="cl-toast-msg"></span>`;
      t.querySelector(".cl-toast-msg").textContent = opts.msg;
      container.appendChild(t);
      // Entrance animation
      requestAnimationFrame(() => requestAnimationFrame(() => t.classList.add("show")));
      const timeout = opts.timeout || (opts.type === "error" ? 5000 : 3500);
      setTimeout(() => {
    ...
    function banner(kind, title, msg) {
      const b = document.createElement("div");
      b.className = `cl-banner ${kind}`;
      b.innerHTML = `<div class="cl-banner-t"></div><div class="cl-banner-m"></div>
        <div class="cl-banner-acts">
          <button class="cl-banner-x">Close</button>
        </div>`;
      b.querySelector(".cl-banner-t").textContent = title;
      b.querySelector(".cl-banner-m").textContent = msg;
      b.querySelector(".cl-banner-x").addEventListener("click", () => b.remove());
      root.appendChild(b);
    }

    // Left-hand CoreLua popup showing the latest screen_capture. Fed from the
    ...
    function showImages(images, toolName) {
      root.querySelectorAll(".cl-shot").forEach((e) => e.remove());
      const wrap = document.createElement("div");
      wrap.className = "cl-shot";
      const hdr = document.createElement("div");
      hdr.className = "cl-shot-hdr";
      const ttl = document.createElement("span");
      ttl.className = "cl-shot-ttl";
      ttl.textContent = `${toolName} · ${images.length} image${images.length > 1 ? "s" : ""}`;
      const close = document.createElement("button");
      close.className = "cl-shot-x";
      close.textContent = "✕";
      ...
      body.className = "cl-shot-body";
      for (const img of images) {
        const el = document.createElement("img");
        el.className = "cl-shot-img";
    ...
    build();
    return { setStatus, setStarted, setStarting, showStop, markStopping, inputCover, toast, banner, showImages, nudgeStart, updateStartGate, refreshSetup, getCustomPrompt, getCustomMcpServers, openMenu: () => openMenuFn && openMenuFn() };
  })();
  ...
  function setChipDetail(item, text) {
    const dt = item && item.querySelector(".cl-chip .cl-chip-dt");
  ...
  function setChipLabel(item, text) {
    const tx = item && item.querySelector(".cl-chip .cl-chip-tx");
  ...
  // CoreLua's own UI). A genuine "regenerate ↻" is always such a click;
  ...
  let _userClickAt = 0;
  document.addEventListener("click", (e) => {
    if (e.isTrusted && !(e.target && e.target.closest && e.target.closest("#cl-root"))) {
  ...
  setInterval(() => {
    ...
    if (P.hasRegenerate !== false &&
      ...
      // the extension's own "■ Stop" is inside #cl-root and never updates
      ...
      if (clickAge < 2500 && clickAfterStop > 400) {
        ...
        if (it) {
          ...
          try {
            it.dataset.zRegenLen = String(P.classifyText(it, ".cl-chip").length);
    ...
    // The bare isHardGenerating() term is gated on a live CoreLua session: on
    ...
    if (gen) {
      ...
      if (item && zphase !== "done" && zphase !== "err" && CLParse.hasToolSignature(reply)) {
        // Live-correct the label as soon as the real name streams in.
        const name = CLParse.toolNameFromText(reply);
  ...
  // A conversation IS a CoreLua session if any rendered turn carries a
  // telltale artefact: the system-prompt marker, an injected tool-result /
  // system-note turn, or a CoreLua command an assistant wrote. Works even
  ...
  function domHasZsSignal() {
    for (const it of P.allItems()) {
      ...
      if (txt.includes(CL.SYS_MARKER)) return true;
  ...
  function syncSessionState() {
    ...
    // via the SITE's own new-chat (not CoreLua's button), the loop is bound to
  ...
  function preHideWholeItems() {
    ...
    if (A.injectHideUntil && Date.now() < A.injectHideUntil) {
      ...
      if (last && !last.classList.contains("cl-hidden") &&
          users.length > (A.injectPreUser || 0)) {
        last.classList.add("cl-hidden");
        A.injectHideUntil = 0; // one-shot: this turn is now masked
        diag("result.prehide", { users: users.length });
      }
    }
    for (const item of items) {
      if (item.classList.contains("cl-hidden")) continue;
      const txt = P.classifyText(item, ".cl-chip");
      if (txt.includes(CL.SYS_MARKER) ||
          (P.isUserItem(item) && CLParse.isInjectedFeedback(txt))) {
        item.classList.add("cl-hidden");
      }
    }
  }
  ...
  setInterval(() => {
    ...
    if (after && P.isUserItem(after) &&
        CLParse.isInjectedFeedback(P.classifyText(after, ".cl-chip"))) return;
    const txt = P.itemText(item);
    if (!CLParse.hasToolSignature(txt)) return;
    ...
    if (!CLParse.parseToolCalls(txt).length) return;
  ...
  log(`CoreLua content script ready (provider: ${P.id})`);
})();
