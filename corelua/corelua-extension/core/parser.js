...
// core/parser.js - CoreLua command parsing. PURE string logic, zero DOM:
...
const CLParse = (() => {
  ...
  // new CoreLua schema ("command"/"params") and the legacy/function-calling
  ...
  // Find EVERY JSON command envelope in the text, not just the first.
  // A reply can legitimately contain several commands when the model broke the
  // "one command per reply" rule - main.js's multiTool feedback then tells it
  // to split them. The old extractToolAnywhere returned only the FIRST call, so
  // a second {"command": ...} in the same reply was silently dropped: never
  // executed, never reported - the model kept going believing it had run (the
  // turn then ended as a plain-text answer and the second command was lost).
  // Each successful parse skips past the whole object (pos = end + 1) so a key
  // quoted INSIDE a string value of a previous command is never re-scanned.
  function extractToolCallsAnywhere(text) {
    const out = [];
    for (const key of ['"command"', '"tool"']) {
      let pos = 0;
      while (true) {
        ...
        try {
          ...
          if (call) { out.push(call); pos = end + 1; continue; }
        } catch {}
        pos = s + 1;
      }
    }
  ...
  function parseToolCalls(r) {
    ...
    while (true) {
      ...
      // Parse JSON command(s) inside the MCP_TOOL block. A block can contain
      // multiple commands separated by bare ###MCP_TOOL### markers (without
      // individual END markers), so we split on START_M first to get each
      // segment, then use extractJson on each. We also fall back to
      // extractToolCallsAnywhere which finds every {"command"/"tool":...}
      // envelope in the body — covering the case where commands are NOT
      // START_M-separated but just listed as consecutive JSON objects.
      const subs = body.split(START_M).map((s) => s.trim().replace(/^(?:json|JSON|Copy|copy)\s*/i, "").trim()).filter(Boolean);
      for (const sub of subs) {
        const p = normalizeCall(extractJson(sub));
        if (p) out.push(p);
      }
      // If the per-segment parse found nothing, try the whole body at once —
      // extractToolCallsAnywhere is string-aware and handles multiple objects.
      if (out.length === 0) {
        const calls = extractToolCallsAnywhere(body);
    ...
    // source (literal \n, \") instead of the decoded code. extractToolCallsAnywhere
    // JSON-decodes it; cleanLuaCall (applied at the end) then strips the markers.
    if (out.length === 0) {
      const calls = extractToolCallsAnywhere(r);
      for (const c of calls) out.push(c);
    }
  ...
  // The assistant emitted a CoreLua command (JSON or a ###LUA### block).
