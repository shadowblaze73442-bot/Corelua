...
// core (core/main.js) only ever talks to the CLProvider interface this file
...
const CLProvider = (() => {
  ...
  // Scope to the SITE's composer only: never match CoreLua's own injected
  // UI (e.g. the settings textarea #cl-set-text in #cl-root). Otherwise on the
  ...
  const getEditor = () => {
    ...
    // returned it - and barMount() then dragged the whole CoreLua bar INTO the
  ...
  function barMount() {
    ...
    if (before && before.id === "cl-bar") before = before.nextElementSibling;
  ...
  function enforceComposer(reason) {
    ...
    // off) without CoreLua reverting their choice every frame.
  ...
  function streamText(item) {
    ...
    const replyTxt = [...item.querySelectorAll(S.markdown)]
      .filter((m) => !m.closest(S.thinking) && !m.closest(".cl-chip"))
      .map((m) => m.textContent)
      .join("");
    return thinkTxt + "\n" + replyTxt;
  }
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…CoreLua: result truncated to fit DeepSeek's ${SEND_CAP}-character ` +
    ...
    try {
      return [...document.querySelectorAll("img")].filter(
        (im) => !im.closest(S.chatItem) &&
          // blob: = pending local preview; the alt (our "corelua_..." filename)
          ...
          (/^blob:/.test(im.getAttribute("src") || "") || /^corelua_/.test(im.getAttribute("alt") || "")));
    } catch { return []; }
  };
  ...
  //   isStarted():bool        - a CoreLua session exists in this chat
  ...
  return {
    ...
    init({ diag: d } = {}) {
      ...
      try { document.documentElement.setAttribute("data-cl-ds-ver", "2026-07_vision-badge-priority"); } catch {}
    },
    // turns
