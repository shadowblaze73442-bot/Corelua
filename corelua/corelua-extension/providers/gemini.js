...
// Exports the same CLProvider interface as providers/deepseek.js; the core
...
const CLProvider = (() => {
  ...
  // Scope to the SITE's composer only: skip CoreLua's own injected UI (the
  // settings textarea #cl-set-text in #cl-root). On login/OAuth pages with no
  ...
  // CoreLua bar stuck on "Agent active" with no way to start a new session.
  ...
  function setInputLock(on) {
    ...
    if (on) ph.setAttribute("data-cl-locked", "1");
    else ph.removeAttribute("data-cl-locked");
  }
  ...
  function readAssistant() {
    ...
    return {
      present: true,
      reply: md ? textWithout(md, ".cl-chip").trim() : "",
  ...
  function truncateForSend(text) {
    ...
    const marker = (what) =>
      `\n\n[…CoreLua: result truncated (${what}) so it can be pasted into ` +
  ...
  // function's real job is applying the .cl-tool-hide classes correctly.
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    for (const mc of replies) {
      ...
      mc.querySelectorAll(S.codeWrap).forEach((cb) => {
        if (cb.closest(".cl-chip")) return;
        if (CMD_SHAPE.test(cb.textContent || "")) {
          cb.classList.add("cl-tool-hide");
          ...
          // rule `message-content.cl-cmd-mask code-block` keeps every
          // recreated block hidden with zero flash.
          mc.classList.add("cl-cmd-mask");
      ...
      [...mc.children].forEach((el) => {
        if (el.classList.contains("cl-chip") || el.querySelector(S.codeWrap)) return;
        const t = el.textContent || "";
        if (t.length < 600 && CMD_SHAPE.test(t)) {
          el.classList.add("cl-tool-hide");
