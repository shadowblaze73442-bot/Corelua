...
chcp 65001 >nul
set "CORELUA_VERSION=1.0"
if not defined CL_BRIDGE_PORT set "CL_BRIDGE_PORT=47170"
set "PYTHONUTF8=1"
title CoreLua Bridge v%CORELUA_VERSION%
cd /d "%~dp0"
...
echo.
echo   === CoreLua Bridge v%CORELUA_VERSION% ===
echo.
echo   Explorer compatibility update enabled
echo   Bridge port: %CL_BRIDGE_PORT%
echo.
...
if not exist "%~dp0corelua-extension\manifest.json" (
    echo   ERROR: the corelua-extension folder is incomplete.
    echo   Extract the complete ZIP before running start.bat.
    call :log "FATAL: corelua-extension\manifest.json is missing."
    pause
    exit /b 1
)
...
REM If a previous bridge is already listening on %CL_BRIDGE_PORT%, say so instead of
...
set "OLDPID="
for /f "tokens=5" %%a in ('netstat -aon ^| findstr :%CL_BRIDGE_PORT% ^| findstr LISTENING 2^>nul') do (
    set "OLDPID=%%a"
)
if defined OLDPID (
    echo         A previous bridge ^(pid !OLDPID!^) is already running on this port.
    echo         Replacing it with this new instance...
    call :log "Killing previous bridge instance (pid !OLDPID!) on port %CL_BRIDGE_PORT%."
    ...
    if defined STILLTHERE (
        echo.
        echo   WARNING: port %CL_BRIDGE_PORT% is still held by pid !STILLTHERE! after trying
        ...
        echo.
        call :log "WARNING: port %CL_BRIDGE_PORT% still held by pid !STILLTHERE! after taskkill."
    )
)
...
echo  ##                                                        ##
echo  ##   CoreLua stops working if you close it. Just       ##
echo  ##   minimize this window and leave it running.           ##
echo  ##                                                        ##
echo  ############################################################
echo.
call :log "Launching bridge.py v%CORELUA_VERSION% with %PY% on port %CL_BRIDGE_PORT%"
%PY% -u "%~dp0bridge.py"
